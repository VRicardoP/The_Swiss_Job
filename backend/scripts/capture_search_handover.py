...
async def run(path):
    ...
    try:
        async with task_session() as db:
            ...
            snapshot["source_database"] = await db.scalar(
            ...
            snapshot["redis_db"] = markers.connection_pool.connection_kwargs.get(
                "db", 0
            )
            ...
            return {
                "seal": snapshot["seal"],
                "searches": len(snapshot["rows"]),
                "pending": sum(
                    not sent
                    for values in snapshot["sent"].values()
                    for sent in values.values()
                ),
                "written": True,
            }
    finally:
...
def main():
    ...
    try:
        ...
        print(
            json.dumps(
                {"verdict": "error", "type": type(exc).__name__, "written": False}
            )
        )
