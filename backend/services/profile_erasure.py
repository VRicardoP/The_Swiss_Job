...
async def deliver_erasure(db, user_id, client_factory=None):
    ...
    try:
        ...
        await db.execute(
            ...
            .values(
                core_profile_id=confirmed_pid,
                core_confirmed_at=erased_at,
                last_error=None,
            )
        )
        await db.commit()
        ...
        await db.commit()
        logger.warning(
            "profile erasure delivery remains pending (%s)", type(exc).__name__
        )
...
async def drain_erasures(session_factory=async_session):
    async with session_factory() as db:
        ids = (
            (
                await db.execute(
                    select(ProfileErasure.user_id)
                    .where(
                        # Unlinked accounts also require a fence against their first CDC event.
                        ProfileErasure.core_confirmed_at.is_(None),
                    )
                    .order_by(
                        ProfileErasure.last_attempt_at.asc().nulls_first(),
                        ProfileErasure.user_id,
                    )
                    .limit(20)
                )
            )
    ...
    async with (client_factory or default_client_factory)() as client:
        while True:
            response = await client.get(
                "/profile-erasures", params={"after": cursor} if cursor else {}
            )
            if response.status_code != 200:
            ...
            if (
                ...
                or body.get("consumer") != "swissjob-shadow"
            ...
            for receipt in body["items"]:
                ...
                erased_at = datetime.fromisoformat(
                    receipt["erased_at"].replace("Z", "+00:00")
                )
                if erased_at.tzinfo is None:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise SystemExit(
            f"erasure reconciliation failed: {type(exc).__name__}"
        ) from None
