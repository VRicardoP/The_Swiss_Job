...
from services.matching.core_client import (
    _FeedItemDTO, _job_view, _match_view, default_client_factory,
)



async def block_feedback_writes(request: Request):
    if settings.FEEDBACK_WRITES_FROZEN and request.method not in {"GET", "HEAD", "OPTIONS"}:
        raise HTTPException(status_code=503, detail="Feedback writes are frozen for cutover")
...
async def require_core_feedback_route(db, user_id):
    ...
    if mode not in {"core_primary", "rollback_pending"}:
        raise CoreUnavailableError("feedback core exige routing autoritativo, sin fallback local")
...
class CoreFeedback:
    ...
    async def _profile(self, user_id):
        ...
        if self._client_factory is default_client_factory and not settings.CORE_CONSUMER_KEY:
    ...
    async def _request(self, method, path, **kwargs):
        ...
        if response.status_code not in (200, 404):
            raise CoreUnavailableError(f"core devolvió {response.status_code} para feedback")
        return response
    ...
    async def _target(self, job_hash):
        ...
        response = await self._request("GET", "/school-jobs",
                                       params={"dedup_key": job_hash, "limit": 2})
        if response.status_code == 200:
            try:
                page = response.json()
                if not isinstance(page["items"], list) or page.get("next_cursor") is not None:
        ...
        response = await self._request("GET", "/listing-references", params={"external_id": job_hash})
    ...
    async def record_implicit_feedback(self, user_id, job_hash, action, duration_ms=None):
        body = {"action": action}
    ...
    async def saved(self, user_id, limit=100, offset=0):
        pid = await self._profile(user_id)
        response = await self._request("GET", f"/profiles/{pid}/feedback", params={"limit": limit, "offset": offset})
        if response.status_code == 404:
            raise CoreUnavailableError("perfil core no disponible")
        try:
            ...
            for row in page.items:
                if row.school_job_id is not None:
                    if not row.job_ref or not re.fullmatch(r"[a-fA-F0-9]{32}", row.job_ref):
                ...
                vacancy = {**(row.content or {}), "id": str(row.vacancy_id) if row.vacancy_id else None,
                           "primary_listing": {"source": row.source, "url": row.url,
                                               "first_seen_at": row.updated_at.isoformat()}}
                item = {"vacancy": vacancy, "state": {"feedback": row.feedback},
                        "evaluation": {"eval_key": f"saved:{pid}:{job_ref}",
                                       "score_final": row.score_final, "scores": row.scores,
                                       "explanation": row.explanation}}
                _FeedItemDTO.model_validate(item)
                source = row.source.removeprefix("legacy:") if row.source else None
                result.append({"match": _match_view(item, job_ref, None),
