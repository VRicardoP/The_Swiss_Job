...
def request_hash(job_hash, doc_type, language):
    return hashlib.sha256(json.dumps([job_hash, doc_type, language], separators=(",", ":")).encode()).hexdigest()


async def _lock_owner(db, user_id):
    if await db.scalar(select(User.id).where(User.id == user_id, User.is_active.is_(True))
...
async def _binding(db, user_id, profile_id):
    ...
    bound = await db.scalar(select(JobhuntProfileMap.core_profile_id).where(
        JobhuntProfileMap.user_id == user_id).with_for_update(read=True))
    if bound != profile_id:
        raise DocumentDeliveryError("profile_binding_changed")


async def enqueue(db, *, operation_id, user_id, profile_id, job_hash, doc_type, language,
                  content, job_title=None, job_company=None):
    """First committed output wins; caller owns commit and routing/freeze checks."""
    if not all(isinstance(value, uuid.UUID) for value in (operation_id, user_id, profile_id)):
    ...
    payload = dict(job_hash=job_hash, doc_type=doc_type, language=language, content=content,
    ...
    await db.execute(insert(DocumentDelivery).values(
        operation_id=operation_id, user_id=user_id, profile_id=profile_id,
        request_hash=identity, payload_hash=digest, payload=payload,
    ).on_conflict_do_nothing(index_elements=[DocumentDelivery.operation_id]))
    row = await db.scalar(select(DocumentDelivery).where(
        DocumentDelivery.operation_id == operation_id).execution_options(populate_existing=True))
    if (row.user_id, row.profile_id, row.request_hash) != (user_id, profile_id, identity):
...
async def operation_status(db, operation_id, user_id, *, expected_request_hash=None):
    row = await db.scalar(select(DocumentDelivery).where(
        DocumentDelivery.operation_id == operation_id, DocumentDelivery.user_id == user_id))
    if row is None:
        return None
    await _binding(db, user_id, row.profile_id)
    if expected_request_hash is not None and row.request_hash != expected_request_hash:
        raise DocumentDeliveryError("operation_conflict")
    return {"operation_id": operation_id, "status": "delivered" if row.delivered_at else "pending",
            "document_id": row.document_id, "error": row.last_error}
    ...
    try:
        async with session_factory() as db:
            ...
            if authority.profile_id != row.profile_id:
                raise DocumentDeliveryError("profile_binding_changed")
            if row.delivered_at:
                return {"operation_id": operation_id, "status": "delivered",
                        "document_id": row.document_id, "error": None}
            ...
            await db.commit()
        sender = (sender_factory or (lambda pid, uid: CoreDocuments(profile_id=pid, user_id=uid)))(pid, user_id)
        ...
        async with session_factory() as db:
            ...
            acknowledged = await db.scalar(update(DocumentDelivery).where(
                DocumentDelivery.operation_id == operation_id, DocumentDelivery.user_id == user_id,
                DocumentDelivery.payload_hash == digest, DocumentDelivery.delivered_at.is_(None),
            ).values(document_id=response.id, delivered_at=func.clock_timestamp(),
                     payload=None, last_error=None).returning(DocumentDelivery.operation_id))
            ...
            await db.commit()
        return {"operation_id": operation_id, "status": "delivered", "document_id": response.id, "error": None}
    except Exception as exc:
        error = str(exc) if isinstance(exc, DocumentDeliveryError) else "delivery_failed"
        ...
        try:
            async with session_factory() as db:
                await db.execute(update(DocumentDelivery).where(
                    DocumentDelivery.operation_id == operation_id, DocumentDelivery.user_id == user_id,
            ...
            pass
        return {"operation_id": operation_id, "status": "pending", "document_id": None, "error": error}
