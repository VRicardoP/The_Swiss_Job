...
class GastrojobScraper(BaseScraper):
    ...
    # VD.4b H5: una página fuera de rango devuelve el contador TOTAL (p. ej.
    # 1104) con 0 ítems — indistinguible de "estructura desconocida" mirando
    # solo esa página. Si CUALQUIER página anterior del MISMO run se parseó
    # bien, un fragmento sin ítems PERO CON contador es fin de paginación, no
    # fallo (sin contador no es un fragmento del portal: fallo VISIBLE aunque
    # el flag esté armado, r2-H1). Se rearma en fetch_jobs porque las
    # instancias pueden reutilizarse entre cosechas (mismo patrón que
    # _run_block_reported en BaseScraper.fetch_jobs).
    _structure_seen_ok = False

    async def fetch_jobs(self, query: str, location: str = "Switzerland") -> list[dict]:
        # Rearme por run del guard H5: sin esto, un run posterior con la
        # estructura rota en la página 1 quedaría silenciado por el éxito de
        # un run anterior — la clase exacta de fallo invisible de VD.4b.
        self._structure_seen_ok = False
    ...
    def parse_listing_page(self, soup: BeautifulSoup) -> list[dict]:
        ...
        for link in soup.select('a[href*="/stellen/stelleninserat/"]'):
        ...
        if stubs:
            self._structure_seen_ok = True
            return stubs

        # 0 stubs: distinguir "vacío legítimo" de "estructura desconocida" con
        # el contador que el fragmento SIEMPRE incluye (fixtures:
        # <div data-mxn-advertisements-count="1104">). El contador se mira
        # ANTES de cortocircuitar con _structure_seen_ok (VD.4b r2-H1): la
        # página fuera de rango real SÍ lo trae, así que un fragmento sin él
        # nunca es fin de paginación — silenciarlo tras una página buena
        # ocultaría un cambio de estructura a mitad de run.
        ...
        elif self._structure_seen_ok:
            # Contador presente en un run que YA parseó ofertas: página fuera
            # de rango (contador TOTAL con 0 ítems) ⇒ fin de paginación
            # legítimo, no un fallo de estructura (ver _structure_seen_ok).
            return stubs
        else:
            announced = count_el.get("data-mxn-advertisements-count", "")
            if announced != "0":
                self._record_structure_failure(
                    f"el portal anuncia {announced!r} ofertas y el parseo no "
                    "reconoció ninguna: estructura desconocida, no '0 ofertas'"
                )
        return stubs
