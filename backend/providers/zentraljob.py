"""Provider for Zentraljob (CH Media Central Switzerland job portal)."""
...
class ZentraljobProvider(BaseJobProvider):
    """Fetch jobs from the Zentraljob CH Media API."""

    SOURCE_NAME = "zentraljob"
    DOMAIN = "zentraljob.ch"
    API_URL = "https://api.zentraljob.ch/public/vacancy/search/"
    MAX_PAGES = 10
    PAGE_SIZE = 20

    async def fetch_jobs(self, query: str, location: str = "Switzerland") -> list[dict]:
        """Fetch jobs from Zentraljob, paginating up to MAX_PAGES."""
        results: list[dict] = []
    ...
    def normalize_job(self, raw: dict) -> dict:
        """Transform a raw Zentraljob API response into the unified job schema."""
