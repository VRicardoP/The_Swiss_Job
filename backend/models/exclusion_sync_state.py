...
class ExclusionSyncState(Base):
    ...
    __table_args__ = (CheckConstraint("version > 0 AND delivered_version >= 0 AND delivered_version <= version", name="ck_exclusion_delivery_version"),)

    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), primary_key=True)
    version: Mapped[int] = mapped_column(BigInteger, nullable=False)
    delivered_version: Mapped[int] = mapped_column(BigInteger, nullable=False, server_default="0")
