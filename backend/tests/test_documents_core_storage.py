...
def document(**changes):
    return {
        ...
        "version": 1,
        "async_state": "ready",
        "context": {"job_title": "Engineer", "job_company": "Example"},
        **changes,
    }
...
def client(monkeypatch, handler, profile=PID):
    ...
    monkeypatch.setattr(
        core_client, "resolve_core_profile_id", AsyncMock(return_value=profile)
    )
    return core_client.CoreDocuments(
        object(),
...
async def test_document_corrupt_200_is_unavailable(monkeypatch, changes):
    core = client(
        ...
        lambda r: httpx.Response(
            200, json={"items": [document(**changes)], "next_cursor": None}
        ),
    )
    with pytest.raises(CoreUnavailableError):
...
async def test_document_pages_and_repeated_cursor_fail_closed(monkeypatch):
    ...
    def handler(req):
        ...
        return httpx.Response(
            200,
            json={
                "items": [a if len(requests) == 1 else b],
    ...
    core = client(
        monkeypatch,
        lambda r: httpx.Response(
            200, json={"items": [document()], "next_cursor": "loop"}
        ),
    )
    with pytest.raises(CoreUnavailableError):
        await core.list(uuid.uuid4(), JOB)
...
async def test_document_create_requires_explicit_operation(monkeypatch):
    core = client(
        monkeypatch, lambda r: pytest.fail("operation id required before HTTP")
    )
    with pytest.raises(CoreUnavailableError):
