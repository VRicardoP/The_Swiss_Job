...
async def test_feedback_all_writes_without_local_match(seeded, db_session, legacy):  # noqa: F811  (la fixture, no una redefinición)
    ...
    def handler(request):
        ...
        return httpx.Response(
            200, json={"profile_id": fake.profile_id, "vacancy_id": vid, **data}
        )
...
async def test_feedback_fails_closed_on_core_error(seeded, db_session, response):  # noqa: F811  (la fixture, no una redefinición)
    ...
    with pytest.raises(CoreUnavailableError):
        await writer(db_session, lambda _: response).submit_feedback(
            user_id, str(uuid.uuid4()), "thumbs_up"
        )
    assert await db_session.scalar(select(func.count()).select_from(MatchResult)) == 0
...
async def test_feedback_saved_historical_page_uses_uuid_and_server_pagination(
                ...
                "items": [
                    {
                        ...
                        "explanation": None,
                        "content": {
                            "title": "Archived native job",
                            "company": "Example",
                            "tags": [],
                        },
                        "url": "https://example.test/archived",
                        "source": "legacy:arbeitnow",
                    }
                ],
            },
        )
    ...
    assert (
        items[0]["match"].job_hash == vid and items[0]["match"].feedback == "thumbs_up"
    )
