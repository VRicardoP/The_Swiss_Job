...
class TestIsPrimaryTeacherJob:
    ...
    def test_matches_german_primarstufe(self):
        assert is_primary_teacher_job(
            "H", "Lehrperson Primarstufe 60-80%", ["schule"]
        ) is True

    def test_matches_french_primaire(self):
        assert is_primary_teacher_job(
            "H", "Enseignant primaire — Genève", None
        ) is True

    def test_matches_from_tags(self):
        assert is_primary_teacher_job("H", "Lehrperson gesucht", ["grundschule"]) is True
...
class TestBuildAlertEmail:
    ...
    def test_subject_has_count_and_plural(self):
        ...
        subject2, _, _ = build_alert_email(
    ...
    def test_html_escapes_content(self):
        _, _, html = build_alert_email(
            [self._job("<script>x</script>", company="A&B")]
        )
