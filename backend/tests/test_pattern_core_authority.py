...
@pytest.mark.asyncio
async def test_core_rejections_are_counted_without_local_jobs(
...
async def test_analyzer_uses_one_authoritative_context(client, db_session, monkeypatch):
    headers, uid, calls = await prepare(
        client, db_session, monkeypatch, httpx.Response(200, json=context())
    )
    ...
    rows = (
        (
            await db_session.execute(
                select(PatternSuggestion).where(PatternSuggestion.user_id == uid)
            )
        )
        .scalars()
        .all()
...
@pytest.mark.parametrize(
    ...
    [
        httpx.Response(503),
        httpx.Response(200, json={"items": [{"feedback": "dismissed"}]}),
    ],
)
async def test_unavailable_context_preserves_pending_suggestions(
    client, db_session, monkeypatch, response
):
    headers, uid, _ = await prepare(client, db_session, monkeypatch, response)
    row = PatternSuggestion(
        user_id=uid,
        suggestion_type="title_pattern",
        pattern="keep",
