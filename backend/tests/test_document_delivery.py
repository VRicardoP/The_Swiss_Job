...
from services.documents.delivery import (
...
def sender_factory(handler):
    return lambda pid, uid: CoreDocuments(
        profile_id=pid,
        user_id=uid,
...
@pytest.mark.anyio
async def test_ack_loss_reuses_exact_persisted_output_without_open_transaction(
    prepared, db_session
):
    ...
    def handler(request):
        ...
        return httpx.Response(
            201, json=receipt(request, prepared["profile_id"], doc_id)
        )
    ...
    assert [req.headers["idempotency-key"] for req in calls] == [
...
async def test_retry_window_and_freeze_send_nothing(prepared, db_session, monkeypatch):
    await db_session.execute(
        update(DocumentDelivery).values(
            first_attempt_at=func.now() - timedelta(hours=23)
        )
    )
    await db_session.commit()
    no_http = sender_factory(
        lambda req: pytest.fail("expired operation must not be resent")
    )
    result = await deliver(
        TestSessionLocal,
        prepared["operation_id"],
        prepared["user_id"],
        sender_factory=no_http,
    )
...
async def test_operation_binding_and_request_are_immutable(prepared, db_session):
    await enqueue(
        db_session, **{**prepared, "content": "Concurrent generation discarded"}
    )
    await db_session.commit()
    db_session.expire_all()
    assert (await db_session.get(DocumentDelivery, prepared["operation_id"])).payload[
        "content"
    ] == "Résumé exact"
    ...
    assert (
        await operation_status(db_session, prepared["operation_id"], uuid.uuid4())
        is None
    )
    await db_session.execute(
        update(JobhuntProfileMap).values(core_profile_id=uuid.uuid4())
    )
    await db_session.commit()
    result = await deliver(
        TestSessionLocal,
        ...
        sender_factory=sender_factory(
            lambda req: pytest.fail("changed binding must not send")
        ),
    )
...
async def test_owner_erased_during_http_is_not_resurrected(prepared):
    async def handler(request):
        async with TestSessionLocal() as db:
            ...
            await db.commit()
        return httpx.Response(
            201, json=receipt(request, prepared["profile_id"], uuid.uuid4())
        )
...
@pytest.mark.parametrize(
    "content",
    ["", "bad\x00value", "bad\ud800value", "x" * 1_000_001],
    ids=["empty", "nul", "surrogate", "oversize"],
)
async def test_invalid_output_is_not_queued(prepared, db_session, content):
    with pytest.raises(DocumentDeliveryError, match="invalid_content"):
        await enqueue(
            db_session, **{**prepared, "operation_id": uuid.uuid4(), "content": content}
        )
