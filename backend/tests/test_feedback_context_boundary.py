...
async def test_invalid_context_fails_closed(client, db_session, monkeypatch, fault):
    ...
    _, uid, calls = await prepare(client, db_session, monkeypatch, httpx.Response(200,json=body))
...
@pytest.mark.asyncio
async def test_whole_context_budget_does_not_extend_interactive_writes(client, db_session, monkeypatch):
    _, uid, _ = await prepare(client, db_session, monkeypatch, httpx.Response(200,json=context()))
    budgets = []
    def response(request):
        budgets.append(request.extensions['timeout']['read'])
        return httpx.Response(200,json=context())
    feedback = CoreFeedback(db_session, client_factory=lambda: httpx.AsyncClient(
        base_url='http://core.test/v1',transport=httpx.MockTransport(response),timeout=5))
