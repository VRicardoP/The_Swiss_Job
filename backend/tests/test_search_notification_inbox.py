...
def search_event(pid, *, push=True):
    ...
    sid = str(uuid.uuid4())
    body["event"].update(
        type="saved_search.matches", aggregate="saved_search", aggregate_id=sid,
        payload={"search_id": sid, "profile_id": str(pid), "search_name": "S" * 200,
                 "match_count": 3, "notify_push": push},
    )
    return body


@pytest.mark.parametrize("push", [False, True])
async def test_search_notification_committed_once_before_sse(client, db_session, monkeypatch, push):
    ...
    monkeypatch.setattr(app.state, "sse_manager", SimpleNamespace(broadcast_to_user=broadcast), raising=False)
    ...
    assert rows[0].data == {"search_id": body["event"]["aggregate_id"],
                            "search_name": "S" * 200, "match_count": 3}
    assert len(calls) == int(push)


@pytest.mark.parametrize("broken", ["owner", "search", "version", "count", "name", "nul", "push"])
async def test_malformed_search_event_has_no_receipt(client, db_session, monkeypatch, broken):
    monkeypatch.setattr(settings, "CORE_INBOX_TOKEN", "synthetic-inbox-token")
    _, _, pid = await owner(client, db_session)
    ...
    assert await db_session.scalar(select(func.count()).select_from(IntegrationInbox)) == 0
    assert await db_session.scalar(select(func.count()).select_from(Notification)) == 0


async def test_sse_failure_does_not_undo_notification_or_duplicate_replay(client, db_session, monkeypatch):
    monkeypatch.setattr(settings, "CORE_INBOX_TOKEN", "synthetic-inbox-token")
    _, _, pid = await owner(client, db_session)
    ...
    assert (await client.post(URL, json=body, headers=HEADERS)).json()["inserted"] is False
...
async def test_failed_notification_rolls_back_receipt_and_retry_creates_it(client, db_session, monkeypatch):
    ...
    assert await db_session.scalar(select(func.count()).select_from(IntegrationInbox)) == 1
    assert await db_session.scalar(select(func.count()).select_from(Notification)) == 1
