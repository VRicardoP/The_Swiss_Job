...
async def test_school_without_canonical_vacancy_can_be_saved_cleared_and_viewed(seeded, db_session):  # noqa: F811  (la fixture, no una redefinición)
    ...
    def handler(request):
        ...
        if request.url.path == "/v1/school-jobs":
            return httpx.Response(200, json={"items": [{"id": jid, "source_ref": ref, "vacancy_id": None}], "next_cursor": None})
        if request.method == "GET":
            return httpx.Response(200, json={"total": 1, "items": [{
                "vacancy_id": None, "school_job_id": jid, "job_ref": ref,
                "feedback": "thumbs_up", "updated_at": "2026-09-19T10:00:00Z",
                "score_final": 0, "scores": {}, "explanation": None,
                "content": {"title": "IT Teacher", "company": "School", "tags": ["school-one"]},
                "url": "https://school.example/jobs/1", "source": "school-one",
            }]})
        assert f"/school-jobs/{jid}/" in request.url.path
        return httpx.Response(200, json={"profile_id": fake.profile_id, "school_job_id": jid, **json.loads(request.content)})
    ...
    assert not any("/vacancies/" in str(r.url) or "/listing-references" in str(r.url) for r in requests)


@pytest.mark.asyncio
@pytest.mark.parametrize("page", [
    {"items": [{"id": str(uuid.uuid4()), "source_ref": "wrong"}], "next_cursor": None},
...
async def test_ambiguous_school_reference_never_falls_back_or_writes(seeded, db_session, page):  # noqa: F811  (la fixture, no una redefinición)
    ...
    with pytest.raises(CoreUnavailableError):
        await writer(db_session, handler).submit_feedback(user_id, "c" * 32, "thumbs_up")
