...
async def test_migration_seeds_active_rules_and_deletions_and_guards_downgrade(client):
    ...
    path = (
        ...
        / "alembic/versions/c4d8e2f60a17_exclusion_delivery.py"
    )
    ...
    async with TestSessionLocal() as session:
        ...
        try:
            ...
            assert states[active_user].exclusions == [
            ...
            assert (
                await session.execute(text("SELECT count(*) FROM exclusion_sync_state"))
            ).scalar_one() == 2
            await session.execute(
                text("UPDATE exclusion_sync_state SET delivered_version = version")
            )
            await connection.run_sync(migrate, "downgrade")
            assert (
                await session.execute(
                    text("SELECT to_regclass('exclusion_sync_state')")
                )
            ).scalar_one() is None
        finally:
