...
async def test_document_for_core_offer_without_local_job(
    ...
    assert (
        response.status_code == {"ok": 200, "missing": 404, "unavailable": 503}[outcome]
    ), response.text
    ...
    assert (
        await client.get(f"/api/v1/documents/item/{body['id']}", headers=_auth(other))
    ).status_code == 404
    assert (
        await client.delete(f"/api/v1/documents/{body['id']}", headers=_auth(token))
    ).status_code == 204


@pytest.mark.parametrize(
    "ref", ["x" * 33, "x" * 36, "{00000000-0000-0000-0000-000000000001}"]
)
