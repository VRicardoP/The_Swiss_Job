...
async def test_concurrent_same_operation_has_one_committed_body(prepared):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def save(content):
        async with TestSessionLocal() as db:
            await start.wait()
            await enqueue(
                db, **{**prepared, "operation_id": operation, "content": content}
            )
            await db.commit()

    tasks = [
        asyncio.create_task(save(content))
        for content in ["first contender", "second contender"]
    ]
    start.set()
    ...
    async with TestSessionLocal() as db:
        assert (
            await db.scalar(
                ...
                .where(DocumentDelivery.operation_id == operation)
            )
            == 1
        )
...
async def test_prepared_delivery_does_not_cross_a_routing_flip(prepared, db_session):  # noqa: F811  (la fixture, no una redefinición)
    ...
    result = await deliver(
        ...
        prepared["user_id"],
        sender_factory=lambda *args: pytest.fail(
            "must not send after authority changed"
        ),
    )
