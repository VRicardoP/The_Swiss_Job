...
| T9 | **CERRADO 2026-09-25** | El refresh vivía 30 días y nada podía retirarlo: `/auth/refresh` emitía tokens nuevos y dejaba el viejo igual de válido, así que «rotación» nombraba algo que no rotaba. Ahora cada refresh lleva `jti` y familia con fila en `refresh_tokens`, se canjea UNA vez, y reutilizar uno canjeado revoca la familia entera (robo detectado). `POST /auth/logout` revoca la familia y contesta 204 siempre —un 401 diría si el token servía—. `users.token_version` viaja en el access (`tv`) y se comprueba en `get_current_user`; `revocar_todas_las_sesiones()` es el corte para cambio de contraseña/baja (hoy sin endpoint: el mecanismo va antes). Ventana de migración: los refresh sin `jti` se aceptan una vez. Núcleo: `create_credential` pone 90 d por defecto (nadie ponía fecha ⇒ ninguna credencial caducaba), `scripts/rotate_credential.py` rota con solape en dos pasos y `jobhunt.credentials.check_health` alerta del solape sin cerrar. 9 pruebas del BFF + 4 del core, y el flujo entero verificado contra el BFF VIVO, no sólo en tests |
| T11 | **CERRADO 2026-09-25** | Los cinco puntos. `shadow.project` deja `core.harvest` (no es sombra: es el motor de matching detrás de un lote de cosecha); la ventana del dispatcher se ancla a **Europe/Zurich**, que es la zona del crontab — anclada en UTC, 11:59 y 12:00 Zurich caían en la MISMA ventana y la idempotencia se comía una cosecha entera (medido, no razonado); las cadencias del beat ganan suelo de 60 s (un `0` heredado convertía observabilidad en un martillo sobre dos núcleos); `_canonical_language` exige forma de código de idioma (`"deutsch"`, `unknown` y un HTML viajaban como idioma de la oferta) y el DTO acota a 5; y `_scope_alerts` lee por fin los contadores de admisión: una fuente que deja de publicar fechas pierde las altas en silencio porque `complete=False` sólo salta si NO HAY NI UNA fecha |
| T12 | **CERRADO 2026-09-25** | Presupuesto TOTAL del recorrido del feed (`CORE_HTTP_TIMEOUT_SECONDS` acotaba cada petición, no la suma: cuatro páginas eran cuatro timeouts con sólo dos workers de gunicorn); el fallo del corpus ESCOLAR degrada en vez de dejar al usuario sin NINGUNA oferta; `lookup` devuelve también lo ya visto para no reencolar títulos; la caché del recorrido se invalida ENTRE procesos por Redis pub/sub (con `-w 2`, limpiarla en un worker dejaba al otro sirviendo lo viejo); y las descripciones se recortan en la caché — los tags NO, porque `school_for_job` empareja el colegio buscando su id entre ellos |
| T0, T13, T15, T16 | PENDIENTES | T13 y T15 son decisiones del propietario |
