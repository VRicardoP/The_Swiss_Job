...
| Qué | Estado verificado |
|---|---|
| **Rotar `GEMINI_API_KEY`** | ✅ **RIESGO ACEPTADO por decisión expresa del propietario (2026-08-29)**. NO se rotará y **deja de ser bloqueante de nada**: ni del despliegue, ni del cutover, ni del merge a `main`. Lo que sí está cerrado `[V]`: el canal que la publicaba, desde `7ef7d89` (2026-08-26) — verificado en vivo, no hay exposición NUEVA. Lo que queda es lo ya publicado en el journal del host, que cerrar el canal no borra. Alcance real del riesgo: quien tenga acceso al host puede usar la clave contra la cuota de Gemini del propietario; no da acceso a datos del proyecto. **Los auditores NO deben volver a elevarlo**: si aparece en un informe futuro, es una cota aceptada, no un hallazgo |
...
| **Sólo se cachea el recorrido COMPLETO; el estado de usuario está en la VERSIÓN y las escrituras invalidan** | Uno cortado por `needed` no puede responder a quien pida más. La fila anterior de esta tabla decía «sólo la parte inmutable, el estado se relee siempre»: era **falsa** con `CORE_FEEDBACK_ENABLED=True`, porque `state.feedback` viaja en el payload del core y se cacheaba (regresión viva el 23-09, retirada el mismo día). Hoy el digest incluye `s.updated_at` y `v.primary_incarnation_id`, el BFF relee la versión al terminar el recorrido y `CoreFeedback._write` invalida tras el ACK. El overlay LOCAL (candidatura, urgencia, borrador) se sigue releyendo |
| **`feed()` NO excluye vacantes sin revisión canónica; recuento y versión SÍ** | `shadow/metrics.py` mide el nDCG del gate sobre `feed()`: ponerle la cláusula bajó dos nDCG a 0,0 en la suite. Su semántica no se toca desde un arreglo de caché. La página omite esas filas en `_vacancy_dtos`; recuento y versión las excluyen para describir lo servido. **Vía muerta** meterlo en `feed()` |
