    ...
    async with SessionLocal() as db:
        if sys.argv[1:] == ["revoke"]:
            ...
            await credentials.revoke_credential(
                db, state["env"]["CORE_CONSUMER_KEY"].split(".")[0]
            )
            await db.commit()
        ...
        await db.execute(
            sa.text(
                "INSERT INTO sources (id,name,tier) VALUES (:s,'rehearsal-feedback-native',0)"
            ),
            {"s": sid},
        )
        await db.execute(
            sa.text(
                "INSERT INTO harvest_scopes (id,source_id,params,tier,enabled) VALUES (:id,:s,'{}',0,false)"
            ),
            {"id": scope, "s": sid},
        )
        raw = RawListing(
            "post-cutover-native",
            "https://feedback-rehearsal.invalid/native-only",
            {
                "title": "Synthetic recovery vacancy",
                "company": "Rehearsal only",
                "description": "Not a real application target",
            },
        )
        ...
        vid = (
            await db.execute(
                sa.text(
                    "SELECT i.vacancy_id FROM source_listings l JOIN source_listing_incarnations i ON i.source_listing_id=l.id WHERE l.source_id=:s AND i.ended_at IS NULL"
                ),
                {"s": sid},
            )
        ).scalar_one()
        cid = (
            await db.execute(
                sa.text(
                    "SELECT id FROM consumers WHERE name='swissjob-shadow' AND active"
                )
            )
        ).scalar_one()
        ...
        try:
            async with async_sessionmaker(source_engine)() as source:
                count = await source.scalar(
                    sa.text("SELECT count(*) FROM jobs WHERE url=:url"),
                    {"url": raw.url},
                )
                ...
                bindings = (
                    await source.execute(
                        sa.text(
                            "SELECT user_id::text,core_profile_id::text FROM jobhunt_profile_map ORDER BY user_id"
                        )
                    )
                ).all()
                source_count = await source.scalar(
                    sa.text("SELECT count(*) FROM match_results")
                )
        finally:
        ...
        private_write(
            state_path,
            {
                ...
                "env": {
                    "DATABASE_URL": SOURCE,
                    "CORE_API_BASE_URL": "http://127.0.0.1:18080/v1",
                    "CORE_FEEDBACK_ENABLED": "true",
                    "CORE_CONSUMER_KEY": key + "." + secret,
            ...
            json.dumps(
                {
                    ...
                    else "private copy operation failed",
                }
            )
        )
