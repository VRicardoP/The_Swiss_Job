...
NEEDED = {'ProbeFailure', '_page_items', '_identities', 'check_catalog',
          'check_matches', 'fingerprint', '_roto', 'self_test'}
    ...
    for fn in tree.body:
        ...
        for n in ast.walk(fn):
            if isinstance(n, ast.If) and any(
                isinstance(s, ast.Raise) and 'ProbeFailure' in ast.dump(s) for s in n.body
            ):
...
print(json.dumps({"guardas": len(objetivos),
                  "mutantes_detectados": len(objetivos) - len(supervivientes),
                  "supervivientes": supervivientes}, indent=1))
