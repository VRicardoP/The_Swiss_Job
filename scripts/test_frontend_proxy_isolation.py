...
def main(image):
    ...
    try:
        for name, marker in (("swissjob-backend", "production"), ("swissjob-backend-r5", "rehearsal")):
            containers.append(docker(
                "run", "-d", "--network", network, "--network-alias", "backend",
                "--network-alias", name, "-e", "PROXY_TEST_MARKER=" + marker,
                "-v", str(fixture.resolve()) + ":/etc/nginx/templates/default.conf.template:ro", image,
            ))
        for host, expected in (("swissjob-backend", "production"), ("swissjob-backend-r5", "rehearsal")):
            frontend = docker("run", "-d", "--network", network, "-e", "SWISSJOB_BACKEND_HOST=" + host, image)
            containers.append(frontend)
            docker("exec", frontend, "nginx", "-t")
            observed = [docker("exec", frontend, "wget", "-q", "-O", "-", "http://127.0.0.1/api/probe") for _ in range(30)]
            assert set(observed) == {expected}, (host, sorted(set(observed)))
        print("PASS: 30/30 production + 30/30 rehearsal, shared ambiguous alias ignored")
    finally:
