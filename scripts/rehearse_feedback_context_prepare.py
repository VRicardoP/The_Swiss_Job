...
async def main():
    assert os.environ['CORE_DATABASE_URL'] == 'postgresql+asyncpg://jobhunt_core@127.0.0.1:5432/core_copy'
    target = ROOT/'feedback-context.private.json'
    async with SessionLocal() as session:
        if sys.argv[1:] == ['revoke']:
            state = private_read(target)
            await credentials.revoke_credential(session, state['env']['CORE_CONSUMER_KEY'].split('.')[0])
        ...
        cid = await session.scalar(sa.text("SELECT id FROM consumers WHERE name='swissjob-shadow' AND active"))
        assert cid is not None
        key, secret = await credentials.create_credential(session, cid, ['matches:read'],
            expires_at=datetime.now(timezone.utc)+timedelta(hours=2))
        state['env']['CORE_CONSUMER_KEY'] = key+'.'+secret
        private_write(target, state)
        await session.commit()
        print(json.dumps({'copy_context_credential':True, 'production_changed':False}))
