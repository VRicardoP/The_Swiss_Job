...
async def fingerprint(table):
    ...
    async with async_session() as session:
        rows = (await session.execute(text(f'SELECT to_jsonb(t) FROM {table} t ORDER BY id'))).scalars().all()
...
async def main():
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app),base_url='http://copy-bff') as client:
        for uid, _ in state['bindings']:
            ...
            response = await client.post('/api/v1/analytics/analyze',headers=headers,json={'min_rejected':2})
        ...
        assert await fingerprint('pattern_suggestions') == proposals, 'failure changed proposals'
    assert await fingerprint('match_results') == before, 'local feedback changed'
    print(json.dumps({'context':counts,'failure_preserves_proposals':True,
        'local_feedback_unchanged':True,'production_changed':False}))
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(json.dumps({'verified':False,'type':type(exc).__name__,
            'check':str(exc) if type(exc) is AssertionError else 'copy-only check failed'}))
