...
def expire(registry, *, now=None, apply=False):
    ...
    def specific_root(root):
        ...
        return (len(root.parts) >= 5 or project_temp) and not root.stat().st_mode & 0o077
    ...
    return {"eligible": len(candidates), "deleted": len(candidates) if apply else 0,
...
def register(registry, path, *, kind, database, created_at, verified=False):
    ...
    entry = {"path": path, "kind": kind, "database": database,
             "created_at": created_at, "expires_at": created_at + limit,
             "size": info.st_size, "sha256": digest, "verified": verified}
...
def main():
    ...
    parser.add_argument("--created-at", type=float, help="original creation time, Unix seconds")
    parser.add_argument("--verified", action="store_true",
                        help="operator attests this backup has passed verification")
    args = parser.parse_args()
    if args.register and (args.apply or not args.kind or not args.database
                          or args.created_at is None):
    ...
    try:
        ...
        if args.register:
            value = register(registry, args.register, kind=args.kind, database=args.database,
                             created_at=args.created_at, verified=args.verified)
