...
async def source_hash():
    async with async_session() as db:
        values = (
            (
                await db.execute(
                    text("SELECT to_jsonb(m) FROM match_results m ORDER BY id")
                )
            )
            .scalars()
            .all()
...
async def saved(client, headers, expected):
    ...
    body = response.json()
    entries = body["data"]
    assert len(entries) == body["total"], (
        "saved list unexpectedly exceeds rehearsal page"
    )
    assert any(x["job_hash"] == state["vacancy_id"] for x in entries) == expected, (
        "native state lost or cross-profile leak"
    )
    return body["total"]
...
async def main():
    ...
    headers = [
        {"Authorization": "Bearer " + create_access_token(uuid.UUID(uid))}
    ...
    async with httpx.AsyncClient(
        ...
        if mode == "exercise":
            ...
            r = await client.post(
                path + "/feedback", headers=headers[0], json={"feedback": "thumbs_up"}
            )
            assert r.status_code == 200
            r = await client.post(
                path + "/implicit", headers=headers[0], json={"action": "opened"}
            )
            assert r.status_code == 200
            r = await client.post(
                path + "/feedback", headers=headers[1], json={"feedback": "thumbs_down"}
            )
            assert r.status_code == 200
        totals = [await saved(client, h, i == 0) for i, h in enumerate(headers)]
        settings.FEEDBACK_WRITES_FROZEN = True
        for method, suffix, body in [
            ("POST", "feedback", {"feedback": "thumbs_up"}),
            ("DELETE", "feedback", None),
            ("POST", "implicit", {"action": "opened"}),
        ]:
            r = await client.request(method, path + "/" + suffix, json=body)
            assert r.status_code == 503, "freeze did not precede auth"
        assert (await client.get("/health/feedback")).json() == {
            "writes": "frozen",
            "writer": "core",
        }
        ...
        if mode == "verify":
            ...
            r = await client.post(
                path + "/feedback", headers=headers[0], json={"feedback": "dismissed"}
            )
    ...
    print(
        json.dumps(
            {
                "phase": mode,
                "saved_native_only_survives": True,
                "separate_users": True,
                "freeze_before_auth": True,
                "frozen_reads_live": True,
                "local_matches_unchanged": True,
                "core_failure_closed": mode == "verify",
                "saved_totals": totals,
            ...
            json.dumps(
                {
                    ...
                    else "copy-only operation failed",
                }
            )
        )
