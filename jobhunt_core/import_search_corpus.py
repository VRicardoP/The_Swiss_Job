...
from jobhunt_core.shadow.projector import (
    JOB_PAYLOAD_MAP,
    _PROJECTOR_LOCK,
    _ensure_legacy_source,
)
...
async def seed_missing_offers(session, source, rows):
    ...
    async with session.begin_nested():
        ...
        mapping = await resolve_vacancies_by_incarnation_urls(
        ...
        if existing:
            raise SearchMigrationError(
                "existing slot requires reconciliation, not missing-offer import"
            )
        ...
        listings = tuple(
            RawListing(
                row["hash"],
                row["url"],
                {target: row[key] for key, target in JOB_PAYLOAD_MAP.items()},
        ...
        actual = (
            await session.execute(
                ...
                {"src": source_id, "ids": [row["hash"] for row in missing]},
            )
        ).all()
        if {(r.external_id, r.url) for r in actual} != {
            (r["hash"], r["url"]) for r in missing
        }:
            raise SearchMigrationError("missing-offer import incomplete")
        provenance = await captured_provenance(session)
        return {
            "resolved": len(rows),
            "inserted_listings": len(missing),
