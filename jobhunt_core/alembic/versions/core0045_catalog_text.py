...
def upgrade():
    ...
    op.execute(f"CREATE INDEX ix_offer_revisions_catalog_title ON {S}.offer_revisions USING gin (lower(coalesce(content->>'title','')) gin_trgm_ops)")
    op.execute(f"CREATE INDEX ix_offer_revisions_catalog_company ON {S}.offer_revisions USING gin (lower(coalesce(content->>'company','')) gin_trgm_ops)")
