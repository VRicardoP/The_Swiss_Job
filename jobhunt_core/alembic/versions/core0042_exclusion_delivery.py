...
def upgrade():
    op.execute(
        f"ALTER TABLE {S}.profiles ADD COLUMN exclusions_version bigint NOT NULL DEFAULT 0 CHECK (exclusions_version >= 0)"
    )
    op.execute(f"""
    ...
    op.execute(
        f"UPDATE {S}.corpus_generation SET generation = generation + 1 WHERE id = 1"
    )


def downgrade():
    op.execute(
        f"DROP TRIGGER trg_corpus_generation_exclusions ON {S}.profile_exclusions"
    )
