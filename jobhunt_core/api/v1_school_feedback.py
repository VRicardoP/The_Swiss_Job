...
async def _write(
    session, principal, profile_id, school_job_id, request, body, implicit
):
    async def handler():
        job = (
            (
                await session.execute(
                    ...
                    {"j": school_job_id, "c": principal.consumer_id},
                )
            )
            .mappings()
            .one_or_none()
        ...
        if result.scalar_one_or_none() is None:
            ...
            raise ApiError(
                ...
                "la referencia pertenece a otro estado escolar",
            )
        if not implicit and job["vacancy_id"] is not None:
            await set_vacancy_feedback(
                session, profile_id, job["vacancy_id"], body.feedback
            )
        field = "action" if implicit else "feedback"
        return 200, {
            "profile_id": str(profile_id),
            "school_job_id": str(school_job_id),
...
async def set_feedback(
    ...
    return await _write(
        session, principal, profile_id, school_job_id, request, body, False
    )
...
async def record_implicit(
    ...
    return await _write(
        session, principal, profile_id, school_job_id, request, body, True
    )
