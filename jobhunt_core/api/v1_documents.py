...
async def _validated_values(session, body):
    ...
    if (
        len(body.content.encode()) > 1_000_000
        or len(json.dumps(body.context, ensure_ascii=False).encode()) > 65536
    ):
        raise ApiError(
            400,
            "document_too_large",
            "contenido o contexto excede la cota de almacenamiento",
        )
    # JSONB expands exponent notation; validate the actual stored representation.
    context_size = (
        await session.execute(
            sa.text("SELECT octet_length(CAST(:context AS jsonb)::text)"),
            {"context": json.dumps(body.context, ensure_ascii=False)},
        )
    ).scalar_one()
    if context_size > 65536:
        raise ApiError(
            400, "document_too_large", "contexto JSONB excede la cota de almacenamiento"
        )
...
async def create_document(
    ...
    body: schemas.DocumentCreateDTO,
    request: Request,
    ...
    if key is None:
        raise ApiError(
            ...
            "el alta de documento exige Idempotency-Key",
        )
    values = await _validated_values(session, body)

    destination = await documents.owner(
        session, profile_id, principal.consumer_id, write=True
    )
    ...
    status, receipt = await run_idempotent(
        session,
        principal,
        f"POST /v1/profiles/{profile_id}/documents",
        request_hash(body.model_dump(mode="json")),
        key,
        handler,
        profile_id=profile_id,
    )
    row = await documents.fetch(
        session, profile_id, uuid.UUID(receipt["id"]), principal.consumer_id
    )
    if row is None:
...
async def create_document_batch(
    profile_id: uuid.UUID,
    body: schemas.DocumentBatchCreateDTO,
    request: Request,
    session=Depends(get_session),
    principal=Depends(require_scope("documents:write")),
):
    ...
    status, receipt = await run_idempotent(
        session,
        principal,
        f"POST /v1/profiles/{profile_id}/documents/batch",
        request_hash(body.model_dump(mode="json")),
        key,
        handler,
        profile_id=profile_id,
    ...
    for did in receipt["ids"]:
        row = await documents.fetch(
            session, profile_id, uuid.UUID(did), principal.consumer_id
        )
        if row is None:
...
async def list_documents(
    ...
    request: Request,
    session=Depends(get_session),
    principal=Depends(require_scope("documents:read")),
    ...
    rows, following = await documents.page(
        session, profile_id, principal.consumer_id, limit, cur, doc_type, source_ref
    )
    return with_etag(
        request,
        schemas.DocumentsPageDTO(
...
async def delete_document(
    ...
    async def handler():
        row = await documents.fetch(
            session, profile_id, document_id, principal.consumer_id
        )
        if row is None:
            raise error_404("documento")
    ...
    status, payload = await run_idempotent(
        session,
        principal,
        f"DELETE /v1/profiles/{profile_id}/documents/{document_id}",
        request_hash({}),
