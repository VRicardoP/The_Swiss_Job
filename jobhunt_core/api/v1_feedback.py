...
from jobhunt_core.feedback import (
    effective_feedback_sql,
    effective_feedback_batch_sql,
    set_vacancy_feedback,
)
...
@router.put(
    "/profiles/{profile_id}/vacancies/{vacancy_id}/feedback", responses=WRITE_RESPONSES
)
...
@router.post(
    "/profiles/{profile_id}/vacancies/{vacancy_id}/implicit", responses=WRITE_RESPONSES
)
async def record_implicit(
    ...
    async def handler():
        vid = await _target(session, profile_id, vacancy_id)
        await _event(
            session, profile_id, vid, "implicit", body.model_dump(exclude_none=True)
        )
        return 200, {
            "profile_id": str(profile_id),
            "vacancy_id": str(vid),
...
async def list_positive_feedback(
    ...
    school_feedback = (
        ...
        + effective_feedback_sql("j.vacancy_id", "a.profile_id")
        + ") END"
    )
    row = (
        await session.execute(
...
async def feedback_context(
    ...
    """
    if (
        await applications.profile_owner(session, profile_id, principal.consumer_id)
        is None
    ):
        raise error_404("perfil")
    ...
    if len(rows) > MAX_FEEDBACK_CONTEXT:
        raise ApiError(
            ...
            "El análisis requiere un contexto completo más pequeño",
        )
