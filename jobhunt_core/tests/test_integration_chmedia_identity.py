    ...
    async def run():
        async with factory() as session:
            await session.execute(
                sa.text("UPDATE sources SET name='ostjob' WHERE id=:s"),
                {"s": made["source"]},
            )
        ...
        async with httpx.AsyncClient(
            ...
            for _ in range(2):
                outcome = await run_scope(
                    sid, get_provider("ostjob"), RawListingSink(), client, factory
                )
        ...
        async with factory() as session:
            ...
            assert all(
                row["apply_url"] == "https://example.org/careers" for row in rows
            )
