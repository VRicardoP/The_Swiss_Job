...
def test_out_of_order_and_duplicate_exclusion_deliveries(db):
    ...
    async def check():
        async with factory() as s:
            owner = (await s.execute(sa.text("SELECT consumer_id FROM profiles WHERE id=:p"), {"p": pid})).scalar_one()
        principal = SimpleNamespace(consumer_id=owner)

        async def put(version, patterns):
            async with factory() as s:
                return await v1.put_profile_exclusions(pid,
                    schemas.ExclusionsWriteDTO(version=version, exclusions=[{"kind": "title_contains", "pattern": p} for p in patterns]),
                    session=s, principal=principal)

        assert (await put(2, [])).version == 2
        assert (await put(1, ["python"])).exclusions == []
        async with factory() as s:
            generation = (await s.execute(sa.text("SELECT generation FROM corpus_generation"))).scalar_one()
        assert (await put(2, [])).version == 2
        async with factory() as s:
            assert (await s.execute(sa.text("SELECT generation FROM corpus_generation"))).scalar_one() == generation
