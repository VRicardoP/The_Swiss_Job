...
def normalized(**metadata):
    ...
    return normalize.normalize_offer(
        name,
        {
...
def test_invalid_text_metadata_does_not_break_normalization(value):
    content = normalized(
        canton=value, language=value, seniority=value, contract_type=value
    )
    assert set(content) == {
        "title",
        "company",
...
def test_empty_metadata_keeps_previous_content_hash():
    ...
    assert normalize.offer_content_hash(content) == normalize.offer_content_hash(
        expected
    )
