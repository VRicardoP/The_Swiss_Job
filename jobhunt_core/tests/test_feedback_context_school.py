...
def test_context_coalesces_linked_school_latest_intent(school_db, school_feedback):  # noqa: F811  (la fixture, no una redefinición)
    ...
    _, _, token = api._issue(
        factory, made, "tenant-match", [*SCOPES, "matches:read", "applications:write"]
    )
    ...
    assert (
        _request(
            factory,
            token,
            path + f"/school-jobs/{job['id']}/feedback",
            "PUT",
            {"feedback": school_feedback},
            "school",
        ).status_code
        == 200
    )
...
def test_feedback_context_requires_read_scope(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    assert (
        api._api(
            factory, f"/v1/profiles/{pid}/feedback-context", token=token
        ).status_code
        == 403
    )
