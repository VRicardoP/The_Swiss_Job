...
def raw(slug="6a2ba788135f3346537f4f73-english-teacher", **changes):
    return {
        ...
        "companyData": {"name": "School"},
        "requiredLocations": "Norway",
        "remoteOfferType": "Full Remote",
        "createdAt": "2026-09-20T00:00:00Z",
        "skills": [{"name": "TEFL"}],
        "salary": {"average": 100, "min": 80, "max": 120, "currency": "EUR"},
        **changes,
    }
...
def test_identity_content_and_raw(monkeypatch):
    ...
    expected = hashlib.md5(
        b"english teacher|school|https://jobgether.com/offer/english-teacher"
    ).hexdigest()
...
def test_relisting_keeps_identity_but_real_url_changes(monkeypatch):
    ...
    second, _ = fetch(
        [{"data": [raw("7a2ba788135f3346537f4f73-english-teacher")], "maxPages": 1}],
        monkeypatch,
    )
...
@pytest.mark.parametrize(
    "body", [None, [], {}, {"data": {}}, {"data": [], "maxPages": True}]
)
...
def test_invalid_neighbor_and_missing_date_preserve_raw(monkeypatch):
    ...
    result, _ = fetch(
        [{"data": [original, raw(slug="../../other")], "maxPages": 1}], monkeypatch
    )
...
def test_non_remote_and_strange_optional_data(monkeypatch):
    ...
    result, _ = fetch(
        [
            {
                "data": [raw(remoteOfferType="On-site", skills=None, salary=[])],
                "maxPages": 1,
            }
        ],
        monkeypatch,
...
def test_ambiguous_relisting_group_is_not_arbitrated_by_order(monkeypatch):
    rows = [
        raw(),
        raw("7a2ba788135f3346537f4f73-english-teacher"),
        raw("other-teacher"),
    ]
    result, _ = fetch([{"data": rows, "maxPages": 1}], monkeypatch)
    assert [r.url for r in result.listings] == [
        "https://jobgether.com/offer/other-teacher"
    ]
...
def test_oversized_page_number_preserves_valid_prefix(monkeypatch):
    result, _ = fetch(
        [{"data": [raw()], "maxPages": 2}, {"data": [], "maxPages": "9" * 5000}],
        monkeypatch,
    )
    assert len(result.listings) == 1 and not result.complete
...
def test_unicode_slug_is_not_lost(monkeypatch):
    result, _ = fetch(
        [{"data": [raw("enseignant-éducateur")], "maxPages": 1}], monkeypatch
    )
...
def test_large_json_integer_does_not_crash_normalization(monkeypatch):
    ...
    result, _ = fetch(
        [{"data": [raw(salary={"average": 10**400, "min": 80})], "maxPages": 1}],
        monkeypatch,
    )
...
def test_wired_date_and_provider():
    ...
    assert (
        publication_date("jobgether", raw()).isoformat() == "2026-09-20T00:00:00+00:00"
    )
...
def test_a_dot_in_the_slug_is_legitimate_but_traversal_is_not(slug, kept, monkeypatch):
    ...
    """
    result, _ = fetch(
        [{"data": [raw(slug), raw("plain-other")], "maxPages": 1}], monkeypatch
    )
    urls = [listing.url for listing in result.listings]
    assert (f"https://jobgether.com/offer/{slug}" in urls) is kept
    assert "https://jobgether.com/offer/plain-other" in urls, (
        "the valid one always survives"
    )
