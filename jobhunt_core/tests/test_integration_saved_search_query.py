...
def test_full_text_and_structured_filters_over_canonical_offers(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    _sink(
        ...
        scope,
        [
            RawListing(key, f"https://search.test/{key}", raw)
    ...
    async def run():
        async with factory() as session:
            await session.execute(
                sa.text(
                    "UPDATE vacancies SET archived_at=clock_timestamp() WHERE id=:id"
                ),
                {"id": ids["archived"]},
            )
            await session.execute(
                sa.text(
                    "UPDATE source_listing_incarnations SET ended_at=clock_timestamp() WHERE vacancy_id=:id"
                ),
                {"id": ids["ended"]},
            )
            ...
            async def matches(filters):
                return {
                    r.vacancy_id for r in await matching_vacancies(session, filters)
                }

            assert await matches({"q": "Python backend"}) == {
                ids[k] for k in ("wanted", "onsite", "unknown")
            }
            all_filters = {
                "q": "Python backend",
...
def test_metadata_update_and_replay_do_not_change_embedding_identity(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    enriched = RawListing(
        listing.external_id,
        listing.url,
        {**base, "canton": "ZH", "salary_min_chf": 80000},
    )
    ...
    assert (
        second.content["canton"] == "ZH" and second.content["salary_min_chf"] == 80000
    )
    _sink(factory, scope, [enriched])
