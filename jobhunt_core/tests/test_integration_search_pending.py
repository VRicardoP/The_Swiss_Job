...
def snapshot(items, sent=()):
    sid = str(uuid.uuid4())
    return {"version": 1, "rows": [{"id": sid}],
        "candidates": {sid: [{"hash": item.external_id, "url": item.url} for item in items]},
...
def test_ambiguous_url_is_not_resolved_by_row_order(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as session:
            await session.execute(sa.text("UPDATE source_listing_incarnations SET url=:url WHERE vacancy_id=:v"),
...
def test_sent_merged_alias_suppresses_pending_winner(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as session:
            await session.execute(sa.text("UPDATE vacancies SET merged_into=:w WHERE id=:l"), {"w": winner,"l":loser})
            assert await resolve_pending(session, frozen) == {sid: []}
    asyncio.run(go())
