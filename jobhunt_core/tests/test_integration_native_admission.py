...
def listing(ref, published):
    return RawListing(ref, f"https://example.test/{ref}", {"created_at": published, "raw": "unchanged"})
...
def test_window_applies_before_sink_and_counts_are_persisted(db):
    ...
    recent, old, absent = listing("recent", now.timestamp()), listing("old", (now-timedelta(days=9)).timestamp()), listing("absent", None)
    ...
    assert admission == {"accepted": 1, "refreshed": 0, "stale": 1, "missing_date": 1, "date_present": 2}
...
def test_absent_dates_refresh_known_items_but_never_report_healthy_empty(db):
    ...
    async def run():
        async with factory() as s:
            await s.execute(sa.text("INSERT INTO source_listings(source_id,external_id,url_normalized) VALUES (:s,'known','https://example.test/known')"), {"s": made["source"]})
            await s.commit()
        sink = CollectSink()
        known = listing("known", None)
        async with httpx.AsyncClient() as http:
            result = await run_scope(sid, WindowProvider((known, listing("new", None))), sink, http, factory)
...
def test_legacy_known_url_is_exact_and_source_scoped(db):
    ...
    sid, = _seed_scopes(factory, made, 1)
    configure(factory, sid, 7)
    known, fragment, foreign = (listing(ref, None) for ref in ("known", "split#B", "foreign"))

    async def run():
        async with factory() as s:
            sources = {}
            for source, url in (("legacy:arbeitnow", known.url),
                ...
                if source not in sources:
                    ...
                    await s.execute(sa.text("INSERT INTO sources(id,name,tier) VALUES (:s,:n,0)"), {"s": sources[source], "n": source})
                src, sl, vac = sources[source], uuid.uuid4(), uuid.uuid4()
                await s.execute(sa.text("INSERT INTO vacancies(id) VALUES (:v)"), {"v": vac})
                await s.execute(sa.text("INSERT INTO source_listings(id,source_id,external_id,url_normalized) VALUES (:i,:s,:ref,:u)"), {"i": sl, "s": src, "u": url, "ref": sl.hex})
                await s.execute(sa.text("INSERT INTO source_listing_incarnations(source_listing_id,vacancy_id,seq,url) VALUES (:l,:v,1,:u)"), {"l": sl, "v": vac, "u": url})
            await s.commit()
        sink = CollectSink()
        async with httpx.AsyncClient() as http:
            result = await run_scope(sid, WindowProvider((known, fragment, foreign)), sink, http, factory)
...
def test_window_change_during_fetch_discards_old_result(db):
    ...
    async def run():
        sink = CollectSink()
        async with httpx.AsyncClient() as http:
            result = await run_scope(sid, Changing((listing("new", None),)), sink, http, factory)
...
def test_admission_sink_failure_does_not_commit_counters(db):
    ...
    async def run():
        async with httpx.AsyncClient() as http:
            result = await run_scope(sid, WindowProvider((listing("new", None),)), FailingSink(), http, factory)
...
def test_admission_empty_feed_stays_healthy_and_omission_keeps_full_feed(db):
    ...
    async def run():
        sink = CollectSink()
        async with httpx.AsyncClient() as http:
            empty = await run_scope(with_window, WindowProvider(()), sink, http, factory)
            full = await run_scope(without_window, WindowProvider((listing("old", None),)), sink, http, factory)
...
def test_cutoff_inclusive_and_fast_path_does_not_query():
    ...
    async def run():
        ...
        admitted = await admit_listings(NoQuery(), "arbeitnow", result, timedelta(days=7), now=now)
        assert admitted.listings == (item,) and admitted.next_cursor["_admission"]["accepted"] == 1
