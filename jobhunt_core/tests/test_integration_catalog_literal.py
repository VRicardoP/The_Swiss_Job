...
def test_indexed_catalog_matches_literal_substring_reference(db):
    ...
    api._enrich_content(
        factory, vacancies[term + "v0"], {"title": term + r" literal%_\ C++ MÜNCHEN"}
    )
    api._enrich_content(
        factory,
        vacancies[term + "v1"],
        {"title": term + " literalZZ C#", "company": term + " MÜNCHEN"},
    )
    ...
    for suffix in (
        ...
        response = api._api(
            factory,
            "/v1/vacancies?" + urlencode({"q": query, "offset": 0, "limit": 100}),
            token=token,
        )
