...
def test_quarantined_school_feedback_handover_is_lossless(school_db):
    ...
    job = _request(factory, token, f"/v1/schools/{monitor['id']}/jobs", "POST", observation, "observe").json()["item"]
    mark_path = f"/v1/profiles/{pid}/school-jobs/{job['id']}/feedback"
    assert _request(factory, token, mark_path, "PUT", {"feedback": "thumbs_up"}, "old-mark").status_code == 200
    ...
    assert _request(factory, token, item_path, "PATCH", {"status": "sent", "draft_content": "private draft"}, "draft", etag).status_code == 200
    before = _rows(factory, "SELECT to_jsonb(a) FROM school_applications a WHERE profile_id=:p", p=pid)
    corpus = _rows(factory, "SELECT count(*) FROM vacancies")
    event = {"action": "opened", "timestamp": "2026-09-19T10:00:00Z"}
    source = [{"id": str(uuid.uuid4()), "user_id": str(pid), "job_hash": observation["dedup_key"],
               "url": observation["url"], "feedback": None, "feedback_implicit": [event, event]}]

    async def run():
        async with factory() as s:
            plan = await prepare_plan(s, consumer=consumer, bindings={str(pid): str(pid)}, rows=source,
                                      recorded_at=datetime.now(timezone.utc).isoformat())
            assert len(plan["changes"]) == 1
            await s.commit()
            await apply_plan(s, plan)
            await s.commit()
            row = (await s.execute(sa.text("SELECT feedback,feedback_implicit,status,draft_content FROM school_applications WHERE profile_id=:p"), {"p": pid})).one()
            ...
            await s.commit()
    asyncio.run(run())
    assert _rows(factory, "SELECT to_jsonb(a) FROM school_applications a WHERE profile_id=:p", p=pid) == before
