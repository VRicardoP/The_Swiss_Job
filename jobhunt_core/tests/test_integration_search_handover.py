...
from jobhunt_core.import_swissjob_searches import (
    SearchMigrationError,
    prepare_plan,
    apply_plan,
)
...
async def fixture_plan(session, made, pending_ids=()):
    ...
    row = {
        "id": sid,
        "user_id": uid,
        "name": "Renamed local search",
        "filters": {"q": "Python"},
        "min_score": 42,
        "notify_frequency": "weekly",
        "notify_push": False,
        "is_active": True,
        "last_run_at": stamp - timedelta(hours=2),
        "total_matches": 31,
        "created_at": stamp - timedelta(days=30),
    }
    plan = await prepare_plan(
        ...
        rows=[row],
        targets={str(sid): str(old_id)},
        pending={str(sid): list(pending_ids)},
...
def test_handover_preserves_public_identity_and_every_value_then_reverts_exactly(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            ...
            row = (
                await s.execute(
                    sa.text("SELECT to_jsonb(s) FROM saved_searches s WHERE id=:id"),
                    {"id": sid},
                )
            ).scalar_one()
            for key in (
            ...
            assert (
                await s.scalar(sa.text("SELECT count(*) FROM saved_search_execution"))
                == 0
            )
            assert (
                await s.scalar(
                    sa.text("SELECT count(*) FROM saved_search_observations")
                )
                == 0
            )
...
def test_old_pending_offer_is_not_lost_and_new_activity_blocks_stale_revert(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE vacancies SET created_at=clock_timestamp()-interval '90 days' WHERE id=:id"
                ),
                {"id": pending},
            )
            await s.commit()
            ...
            await s.execute(
                sa.text(
                    "UPDATE saved_search_execution SET enabled=true WHERE saved_search_id=:id"
                ),
                {"id": sid},
            )
            await s.commit()
...
def test_changed_target_aborts_without_partial_import(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            ...
            await s.execute(
                sa.text(
                    "UPDATE saved_searches SET name='concurrent edit' WHERE id=:id"
                ),
                {"id": old},
            )
            await s.commit()
            ...
            assert (
                await s.scalar(
                    sa.text("SELECT name FROM saved_searches WHERE id=:id"), {"id": old}
                )
                == "concurrent edit"
            )
            assert (
                await s.scalar(
                    sa.text("SELECT id FROM saved_searches WHERE id=:id"), {"id": sid}
                )
                is None
            )
            assert (
...
def test_explicit_identity_mapping_preserves_homonymous_searches(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            ...
            old2 = await saved_searches.create(
                s, profile_id=pid, destination=tenant, values={"name": "Search"}
            )
            await s.commit()
            ...
            plan = await prepare_plan(
                ...
                bindings={str(uid): str(pid)},
                rows=rows,
                targets={str(a): str(old1), str(b): str(old2)},
                pending={str(a): [], str(b): []},
                recorded_at=stamp.isoformat(),
            )
            await s.commit()
            ...
            assert dict(
                (
                    await s.execute(
                        sa.text(
                            "SELECT id,min_score FROM saved_searches WHERE profile_id=:id"
                        ),
                        {"id": pid},
                    )
                ).all()
