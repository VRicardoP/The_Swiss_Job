...
JOB = {
    ...
    "companyName": "Bank AG",
    "location": "Zürich",
    "description": "<p>Risk &amp; controlling</p>",
...
def test_accepted_identity_forms_match_the_portal(job_id, expected, monkeypatch):
    result, _ = fetch(
        [page([{**JOB, "jobId": None, **job_id}]), page([])], monkeypatch=monkeypatch
    )
...
def test_blank_company_degrades_exactly_like_a_missing_one(monkeypatch):
    ...
    blank, _ = fetch(
        [page([{**JOB, "companyName": "  <b></b> "}]), page([])],
        monkeypatch=monkeypatch,
    )
    missing, _ = fetch(
        [page([{**JOB, "companyName": None}]), page([])], monkeypatch=monkeypatch
    )
    assert (
        normalize_offer("financejobs", blank.listings[0].payload)["company"]
        == normalize_offer("financejobs", missing.listings[0].payload)["company"]
        == "Unknown"
    )
...
def test_historic_props_shape_is_still_read(monkeypatch):
    result, _ = fetch(
        [page([JOB], ("initialProps", "pageProps", "jobsSSR")), page([])],
        monkeypatch=monkeypatch,
    )
...
def test_page_full_of_unparseable_offers_is_a_failure_too(monkeypatch):
    with pytest.raises(ProviderResponseError):
        fetch(
            [
                page([{"jobId": None, "title": "x"}, {"jobId": None, "title": "y"}]),
                page([]),
            ],
            monkeypatch=monkeypatch,
        )
