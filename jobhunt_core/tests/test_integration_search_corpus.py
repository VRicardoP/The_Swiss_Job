...
def rows(source):
    ...
    row.update(
        hash=uuid.uuid4().hex,
        source=source,
        url="https://missing-search.test/" + uuid.uuid4().hex,
        apply_url=None,
        is_active=True,
        duplicate_of=None,
    ...
    async def go():
        async with factory() as session:
            ...
            content = await session.scalar(
                sa.text("SELECT content FROM offer_revisions WHERE vacancy_id=:v"),
                {"v": uuid.UUID(first["provenance"]["vacancies"][0])},
            )
            assert content["language"] == "en"
            assert not await session.scalar(
                sa.text("SELECT enabled FROM harvest_scopes WHERE id=:id"),
                {"id": made["scopes"][0]},
            )
...
@pytest.mark.parametrize(
    "defect", ["inactive", "duplicate", "missing_field", "quarantined"]
)
    ...
    async def go():
        async with factory() as session:
            ...
            assert await session.scalar(
                sa.text("SELECT archived_at IS NOT NULL FROM vacancies WHERE id=:id"),
                {"id": vid},
            )
