...
def test_batch_roundtrip_canonical_replay_and_new_generation(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    receipt = _rows(
        f, "SELECT route,response FROM idempotency_records WHERE key='batch-1'"
    )[0]
...
def test_batch_second_outbox_failure_rolls_back_everything(db, monkeypatch):  # noqa: F811  (la fixture, no una redefinición)
    ...
    assert (
        post(f, token, pid).status_code == 201
    )  # failed attempt did not poison the key
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "items": [
                {"doc_type": "cv", "content": "ok"},
                {"doc_type": "cover_letter", "content": "bad\x00"},
            ]
        },
        {
            "items": [
                {"doc_type": "cv", "content": "ok"},
                {"doc_type": "cover_letter", "content": "é" * 500001},
            ]
        },
        {
            "items": [
