...
pytestmark = pytest.mark.skipif(
    not os.getenv("CORE_ADMIN_DATABASE_URL"), reason="requires PostgreSQL"
)
...
def test_resolve_listing_reference_and_ambiguity(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    path = "/v1/listing-references?" + urlencode(
        {"external_id": ref.external_id, "source": ref.source}
    )
    ...
    assert (
        api._api(
            factory,
            "/v1/listing-references?external_id=nonexistent-test-alias",
            token=token,
        ).status_code
        == 404
    )
    ...
    async def collide():
        async with factory() as session:
            ...
            await session.execute(
                ...
                {"id": sid, "name": "alias-collision-" + sid.hex},
            )
            await session.execute(
                sa.text(
                    "INSERT INTO source_listings(id,source_id,external_id,url_normalized) VALUES(:id,:s,:ref,:url)"
                ),
                {
                    "id": lid,
                    "s": sid,
                    "ref": ref.external_id,
                    "url": "https://example.test/alias-" + lid.hex,
                },
            )
            await session.execute(
                sa.text(
                    "INSERT INTO source_listing_incarnations(id,source_listing_id,vacancy_id,seq,url) VALUES(:id,:l,:v,1,:u)"
                ),
                {
                    "id": uuid.uuid4(),
                    "l": lid,
                    "v": refs[1].vacancy_id,
                    "u": "https://example.test/alias-" + lid.hex,
                },
            )
    ...
    assert api._api(factory, path, token=token).json() == {
        "vacancy_id": str(ref.vacancy_id)
    }
...
def test_listing_reference_rejects_unstorable_query(db, field):  # noqa: F811  (la fixture, no una redefinición)
    ...
    _, _, token = api._issue(
        factory, created, "reference-validation", ["vacancies:read"]
    )
    params = {"external_id": "valid", field: "bad\x00reference"}
    response = api._api(
        factory, "/v1/listing-references?" + urlencode(params), token=token
    )
