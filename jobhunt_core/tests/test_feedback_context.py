...
def test_feedback_context_includes_native_rejection_and_unmarked_history(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    assert any(
        row["identity"] == f"vacancy:{vid}"
        and row["feedback"] in ("thumbs_down", "dismissed")
        for row in items
    )
    assert any(row["feedback"] is None for row in items)
    assert all(
        isinstance(row["title"], str) and isinstance(row["tags"], list) for row in items
    )
...
def test_archival_does_not_erase_rejection_context(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    assert response.status_code == 200
    assert any(
        row["identity"] == f"vacancy:{vid}" and row["feedback"] == "dismissed"
