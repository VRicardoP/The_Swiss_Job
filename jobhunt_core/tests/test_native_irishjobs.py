...
from jobhunt_core.harvest.providers.native_irishjobs import (
    IrishJobsProvider, canonical_identity_url)

ITEM = {"id": 107803777, "title": "Lead MS Fabric Architect", "companyName": "NTT Data",
        "location": "Dublin", "url": "/job/lead-ms-fabric-architect/ntt-data-job107803777",
        "textSnippet": "<p>Azure &amp; Fabric</p>", "salary": "€60,000 - €70,000 per year",
        "datePosted": "2026-09-20T08:00:00Z", "companyLogoUrl": "/CompanyLogos/"}


def page(items, extra_script=""):
    blob = json.dumps({"searchResults": {"items": items, "total": len(items)}})
    return (b"<html><script>var x = 1;</script>"
            + extra_script.encode()
            + b'<script>window.__PRELOADED_STATE__["app-unifiedResultlist"] = '
            + blob.encode() + b";</script></html>")
...
def test_offer_keeps_platform_identity_real_url_and_canonical_content(monkeypatch):
    ...
    assert listing.url == ("https://www.irishjobs.ie"
                           "/job/lead-ms-fabric-architect/ntt-data-job107803777")
    assert listing.payload == ITEM
    content = normalize_offer("irishjobs", listing.payload)
    assert content["title"] == "Lead MS Fabric Architect" and content["company"] == "NTT Data"
    assert content["location"] == "Dublin" and content["remote"] is True
    assert content["salary"] == "€60,000 - €70,000 per year", "EUR text, never a CHF amount"


def test_reediting_the_slug_is_a_re_sighting_not_a_clone():
    """Measured in production: 40 clone rows out of 919 before this rule."""
    before = "https://www.irishjobs.ie/job/lead-ms-fabric-architect/ntt-data-job107803777"
...
def test_the_two_hosts_are_one_source_deduplicated_by_platform_id(monkeypatch):
    ...
    result, seen = fetch({"www.irishjobs.ie": [page([ITEM]), page([])],
...
def test_unusable_url_drops_only_that_offer(url, monkeypatch):
    ...
    result, _ = fetch({"www.irishjobs.ie": [page(rows), page([])]}, monkeypatch=monkeypatch)
...
def test_url_is_rebuilt_without_query_or_fragment(monkeypatch):
    ...
    result, _ = fetch({"www.irishjobs.ie": [
        page([{**ITEM, "url": "/job/x-job1?utm_source=x#top"}]), page([])]},
        monkeypatch=monkeypatch)
...
def test_anchor_ignores_read_only_references_and_the_other_blob(monkeypatch):
    noise = ('<script>var a = window.__PRELOADED_STATE__["app-unifiedResultlist"];'
             'window.__PRELOADED_STATE__["google-onetap"] = {"searchResults": '
             '{"items": [{"title": "WRONG", "url": "/job/wrong-job9"}]}};</script>')
    result, _ = fetch({"www.irishjobs.ie": [page([ITEM], noise), page([])]},
                      monkeypatch=monkeypatch)
...
def test_brace_inside_a_json_string_does_not_truncate_the_literal(monkeypatch):
    result, _ = fetch({"www.irishjobs.ie": [
        page([{**ITEM, "title": 'Architect } with brace'}]), page([])]},
        monkeypatch=monkeypatch)
...
def test_empty_items_is_a_legitimate_empty_page(monkeypatch):
    ...
    result, _ = fetch({"www.irishjobs.ie": [page([])], "www.jobs.ie": [page([])]},
                      monkeypatch=monkeypatch)
...
def test_page_full_of_unparseable_items_is_a_failure(monkeypatch):
    with pytest.raises(ProviderResponseError):
        fetch({"www.irishjobs.ie": [page([{"title": "x"}, {"url": "/job/y-job1"}]),
                                    page([])]}, monkeypatch=monkeypatch)


def test_late_failure_preserves_the_valid_prefix(monkeypatch):
    result, _ = fetch({"www.irishjobs.ie": [page([ITEM]), httpx.Response(503)]},
                      monkeypatch=monkeypatch)
...
def test_declared_budget_bounds_each_host(monkeypatch):
    ...
    result, seen = fetch({"www.irishjobs.ie": many, "www.jobs.ie": many},
                         {"max_pages": 2}, monkeypatch)
