...
def test_feedback_roundtrip_dismiss_clear_and_idempotent_event(db):
    ...
    kwargs = dict(token=token, method="PUT", json_body={"feedback": "thumbs_down"}, headers={"Idempotency-Key": "reject-one"})
    one = api._api(factory, path, **kwargs)
    two = api._api(factory, path, **kwargs)
    assert one.status_code == two.status_code == 200
    assert one.json() == two.json()
    state = rows(factory, "SELECT * FROM profile_vacancy_state WHERE profile_id=:p AND vacancy_id=:v", p=pid, v=vid)[0]
    assert state["feedback"] == "thumbs_down" and state["dismissed_at"] is not None
    assert state["current_eval_id"] == before["current_eval_id"]
    assert len(rows(factory, "SELECT id FROM profile_vacancy_events WHERE profile_id=:p AND kind='feedback'", p=pid)) == 1
    assert api._api(factory, path, token=token, method="PUT", json_body={"feedback": None}).status_code == 200
    state = rows(factory, "SELECT feedback,dismissed_at FROM profile_vacancy_state WHERE profile_id=:p AND vacancy_id=:v", p=pid, v=vid)[0]
...
def test_feedback_scopes_and_ownership_fail_before_writing(db):
    ...
    _, _, other = api._issue(factory, db[1], "other-feedback-tenant", ["applications:write"])
...
def test_implicit_append_is_idempotent_and_does_not_overwrite_explicit(db):
    ...
    for key in ("open", "open", "view"):
        response = api._api(factory, base + "/implicit", token=token, method="POST", json_body={"action": "opened"}, headers={"Idempotency-Key": key})
        assert response.status_code == 200
    assert len(rows(factory, "SELECT id FROM profile_vacancy_events WHERE profile_id=:p AND kind='implicit'", p=pid)) == 2
...
def test_feedback_absent_vacancy_and_invalid_values_are_not_success(db):
    ...
    path = f"/v1/profiles/{pid}/vacancies/{vid}/feedback"
    for body in ({"feedback": "arbitrary"}, {"feedback": "thumbs_up", "profile_id": str(uuid.uuid4())}):
...
def test_feedback_conflicting_replay_does_not_change_state(db):
    ...
    assert api._api(factory, path, json_body={"feedback": "thumbs_up"}, **args).status_code == 200
    assert api._api(factory, path, json_body={"feedback": "thumbs_down"}, **args).status_code == 409
    assert rows(factory, "SELECT feedback FROM profile_vacancy_state WHERE profile_id=:p AND vacancy_id=:v", p=pid, v=vid)[0]["feedback"] == "thumbs_up"
...
def test_archived_feedback_can_be_cleared_without_reopening_corpus(db):
    ...
    asyncio.run(archive())
    assert api._api(factory, path, token=token, method="PUT", json_body={"feedback": None}).status_code == 200
    assert rows(factory, "SELECT archived_at FROM vacancies WHERE id=:v", v=vid)[0]["archived_at"] is not None
...
def test_feedback_events_are_erased_with_profile_and_replay_stays_closed(db):
    ...
    assert api._api(factory, base + "/implicit", token=token, method="POST", json_body={"action": "opened"}).status_code == 200
    assert api._api(factory, f"/v1/profiles/{pid}", token=token, method="DELETE").status_code == 200
    assert rows(factory, "SELECT id FROM profile_vacancy_events WHERE profile_id=:p", p=pid) == []
    assert rows(factory, "SELECT profile_id FROM profile_vacancy_state WHERE profile_id=:p", p=pid) == []
...
def test_positive_feedback_listing_survives_archive_and_missing_evaluation(db):
    ...
    async def archive():
        async with factory() as session:
            await session.execute(sa.text("UPDATE vacancies SET archived_at=now() WHERE id=:v"), {"v": vid})
            await session.execute(sa.text("UPDATE profile_vacancy_state SET current_eval_id=NULL WHERE profile_id=:p AND vacancy_id=:v"), {"p": pid, "v": vid})
            await session.commit()
    asyncio.run(archive())
    ...
    assert api._api(factory, f"/v1/profiles/{pid}/feedback?offset=1", token=token).json() == {"total": 1, "items": []}
    _, _, other = api._issue(factory, db[1], "other-feedback-reader", ["matches:read"])
    assert api._api(factory, f"/v1/profiles/{pid}/feedback", token=other).status_code == 404
