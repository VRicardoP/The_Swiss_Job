...
def test_offset_filters_empty_total_and_etag(db):
    ...
    api._enrich_content(
        factory,
        vacancies[term + "v0"],
        {"remote": True, "location": "Zurich, Switzerland"},
    )
    ...
    cached = api._api(
        factory, base, token=token, headers={"If-None-Match": response.headers["etag"]}
    )
    assert cached.status_code == 304
    empty = api._api(
        factory, base.replace("city=zurich", "city=missing"), token=token
    ).json()
...
def test_offset_rejects_mixed_cursor_and_negative_offset(db):
    ...
    assert (
        api._api(factory, base + "&offset=0&cursor=" + cursor, token=token).status_code
        == 400
    )
    for offset in (-1, 9223372036854775808):
        assert (
            api._api(factory, base + f"&offset={offset}", token=token).status_code
            == 400
        )
