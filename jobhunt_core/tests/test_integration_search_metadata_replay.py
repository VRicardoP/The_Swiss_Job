...
def test_existing_raw_replays_to_enriched_canonical_without_duplicate_raw(
    ...
    def old_picker(payload):
        return {
            k: v
            for k, v in current_picker(payload).items()
            if k in normalize.CONTENT_FIELDS
        }
