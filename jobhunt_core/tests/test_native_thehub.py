...
def item(i=1):
    return {"id": f"{i:024x}", "title": "English teacher", "company": {"name": "School"},
            "isRemote": True}


def detail(i=1):
    return {**item(i), "description": "<p>English TEFL</p>", "location": {"address": "Remote"},
...
def fetch(pages, details=None, monkeypatch=None):
    ...
    async def run():
        def transport(request):
            ...
            return httpx.Response(value) if type(value) is int else httpx.Response(200, content=json.dumps(value).encode())
...
def test_detail_is_preserved_and_normalized(monkeypatch):
    ...
    assert row.payload == detail() and row.url == "https://thehub.io/jobs/" + item()["id"]
    content = normalize_offer("thehub", row.payload)
    assert content["description"] == "English TEFL" and content["location"] == "Remote"


@pytest.mark.parametrize("broken", [503, None, [], {}, {"id": "000000000000000000000099", "description": "WRONG"}])
def test_failed_detail_is_not_a_destructive_sparse_refresh(broken, monkeypatch):
    result, _ = fetch([{"docs": [item(), item(2)], "pages": 1}],
                      {item()["id"]: broken}, monkeypatch)
    assert not result.complete and result.error
    assert len(result.listings) == 1 and result.listings[0].payload["id"] == item(2)["id"]


@pytest.mark.parametrize("body", [None, [], {}, {"docs": "bad", "pages": 1}, {"docs": [], "pages": 2}])
...
def test_invalid_object_id_never_requests_detail(monkeypatch):
    result, requests = fetch([{"docs": [item(), {**item(2), "id": "../../admin"}], "pages": 1}],
...
def test_final_page_and_empty_contract(monkeypatch):
    ...
    result, requests = fetch([{"docs": [item()], "pages": 2}, {"docs": [item(2)], "pages": 2}],
                             monkeypatch=monkeypatch)
