...
def test_quarantined_school_feedback_preserves_draft_and_has_no_fake_vacancy(school_db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    job = _request(f, token, f"/v1/schools/{monitor['id']}/jobs", "POST", observation, "observe").json()["item"]
    assert job["vacancy_id"] is None
    before = _rows(f, "SELECT count(*) FROM vacancies")
    base = f"/v1/profiles/{pid}/school-jobs/{job['id']}"
    put = _request(f, token, base + "/feedback", "PUT", {"feedback": "thumbs_up"}, "like")
    ...
    assert _request(f, token, f"/v1/profiles/{pid}/feedback?offset=1").json() == {"items": [], "total": 1}
    ...
    draft = _request(f, token, path, "PATCH", {"status": "sent", "draft_content": "private draft"}, "draft", current.headers["etag"])
    assert draft.status_code == 200, draft.text
    for key in ("opened", "opened", "another-open"):
        response = _request(f, token, base + "/implicit", "POST", {"action": "opened"}, key)
...
def test_school_feedback_ownership_and_scope_before_mutation(school_db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    for actor, profile in ((other, pid), (other, other_pid)):
        response = _request(f, actor, f"/v1/profiles/{profile}/school-jobs/{job['id']}/feedback", "PUT", {"feedback": "thumbs_up"}, "cross-tenant")
        assert response.status_code == 404, response.text
    reader, _, reader_pid = _seed_profile(f, made, ["schools:read"])
    response = _request(f, reader, f"/v1/profiles/{reader_pid}/school-jobs/{job['id']}/feedback", "PUT", {"feedback": "thumbs_up"}, "read-only")
...
def test_rejected_quarantine_stays_hidden_when_linked_and_clear_restores_feed(school_db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    job = _request(f, token, f"/v1/schools/{monitor['id']}/jobs", "POST", _observation(publish_missing=False), "observe").json()["item"]
    base = f"/v1/profiles/{pid}/school-jobs/{job['id']}"
    assert _request(f, token, base + "/feedback", "PUT", {"feedback": "thumbs_down"}, "reject").status_code == 200
    ...
    assert _request(f, token, canonical, "PUT", {"feedback": None}, "clear").status_code == 200
    assert vid in asyncio.run(feed_ids())
    assert _request(f, token, base + "/feedback", "PUT", {"feedback": "thumbs_up"}, "positive").status_code == 200
    saved = _request(f, token, f"/v1/profiles/{pid}/feedback").json()
    assert saved["total"] == 1  # never duplicate the canonical and school mark
    assert saved["items"][0]["school_job_id"] == job["id"]
    assert _request(f, token, canonical, "PUT", {"feedback": "dismissed"}, "canonical-reject").status_code == 200
