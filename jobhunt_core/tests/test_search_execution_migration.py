...
pytestmark = pytest.mark.skipif(not os.getenv("CORE_ADMIN_DATABASE_URL"), reason="requires PG")
...
def test_core0050_incremental_upgrade_and_guarded_downgrade():
    ...
    try:
        ...
        execute("INSERT INTO saved_searches(profile_id,name) SELECT id,'existing' FROM profiles")
        ...
        assert execute("SELECT to_jsonb(s) FROM saved_searches s") == before
        run_alembic(url, "upgrade", "core0050")
        execute("INSERT INTO saved_search_execution(saved_search_id,contract,notify_since) SELECT id,'swissjob-v1',clock_timestamp() FROM saved_searches")
