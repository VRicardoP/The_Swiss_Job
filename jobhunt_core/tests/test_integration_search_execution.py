...
async def new_search(
    session, made, *, filters=None, configured=True, enabled=True, name="Search"
):
    ...
    sid = await saved_searches.create(
        session,
        profile_id=pid,
        destination=tenant,
        values={
            "name": name,
            "filters": {"q": "Python"} if filters is None else filters,
    ...
    if configured:
        await execution.configure_execution(
            session,
            sid,
...
def listing(ref, title="Python developer"):
    return RawListing(
        ref,
        f"https://search-run.test/{ref}",
        {"title": title, "company_name": "Example"},
    )


async def run(session, sid, tenant, *, force=True):
    result = await execution.execute_search(
        session, sid, destinations={tenant}, force=force
    )
    await session.commit()
    return result
...
def test_old_offers_and_filter_edits_do_not_manufacture_novelty(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE vacancies SET created_at=clock_timestamp()-interval '30 days' WHERE id=:v"
                ),
                {"v": old},
            )
            sid, _, tenant, _ = await new_search(s, made)
            assert (await run(s, sid, tenant))["matches"] == 1
            row = await saved_searches.fetch_owned(
                s, sid, made["consumers"][0], for_update=True
            )
...
def test_no_execution_without_explicit_active_authority(db, condition):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            sid, pid, tenant, _ = await new_search(
                s,
                made,
            ...
            if condition == "inactive_search":
                await s.execute(
                    sa.text("UPDATE saved_searches SET is_active=false WHERE id=:id"),
                    {"id": sid},
                )
                ...
                await s.execute(
                    sa.text(
                        "UPDATE profiles SET projection_active=false,projection_version=1,projection_hash=:hash WHERE id=:id"
                    ),
                    {"id": pid, "hash": "a" * 64},
                )
            elif condition == "inactive_consumer":
                await s.execute(
                    sa.text("UPDATE consumers SET active=false WHERE id=:id"),
                    {"id": made["consumers"][0]},
                )
            await s.commit()
            ...
            st = await state(s, sid)
            assert st.last_run_at is None and (
                st.observed,
                st.total_matches,
                st.events,
            ) == (0, 0, 0)
                ...
                async with asyncio.timeout(5):
                    while True:
                        ...
                        blocked = await monitor.scalar(
                            sa.text("SELECT :blocker = ANY(pg_blocking_pids(:waiter))"),
...
def test_schedule_and_manual_run_and_authority_disable(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def go():
        async with factory() as s:
            ...
            await execution.configure_execution(
                s, sid, contract=execution.CONTRACT, notify_since=since, enabled=False
            )
            await s.commit()
            ...
            with pytest.raises(ValueError, match="already fixed"):
                await execution.configure_execution(
                    ...
                    sid,
                    contract=execution.CONTRACT,
                    notify_since=since - timedelta(days=1),
                    enabled=True,
                )
            await s.rollback()
        ...
        async with factory() as s:
            ...
            for table in ("saved_search_execution", "saved_search_observations"):
                assert (
                    await s.scalar(
                        sa.text(
                            f"SELECT count(*) FROM {table} WHERE saved_search_id=:id"
                        ),
                        {"id": sid},
                    )
                    == 0
