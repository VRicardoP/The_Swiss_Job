...
@pytest.mark.parametrize("cantons,city,expected", [
...
def test_chmedia_exposes_the_same_canton_as_the_retiring_writer(
    ...
    raw = {"id": 1, "title": "Lehrperson", "company": {"name": "Schule"},
           "workplaceCity": city, "cantons": cantons, "activity": "<p>Unterricht</p>"}
...
def test_publicjobs_exposes_only_the_two_letter_region(region, expected):
    PublicJobsProvider()
    raw = {"path": "/job/1", "title": "Sachbearbeiter", "contactCompany": "Gemeinde",
           "workingAddressCity": "Genève", "workingAddressRegion": region}
...
def test_zebis_extracts_the_canton_from_its_description(description, expected):
    listing = rss_fetch("zebis", zebis_body(
        "https://www.zebis.ch/stellen/lehrperson-1", description=description)).listings[0]
...
def test_canton_never_leaks_into_the_embedded_text():
    ...
    content = normalize_offer("ostjob", {
        "id": 1, "title": "Lehrperson", "company": {"name": "Schule"},
        "workplaceCity": "Wil", "cantons": ["St. Gallen"], "activity": "<p>Unterricht</p>"})
