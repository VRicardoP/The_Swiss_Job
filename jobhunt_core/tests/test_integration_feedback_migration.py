...
def test_feedback_incremental_upgrade_and_non_destructive_downgrade():
    ...
    try:
        ...
        execute(
            "INSERT INTO schools(id,school_key,name,country) VALUES (gen_random_uuid(),'CH:test','Test','CH')"
        )
        execute(
        ...
        execute(
            "INSERT INTO school_applications(id,profile_id,consumer_id,monitor_id,source_ref,status,draft_content) "
        ...
        execute(
            "INSERT INTO profile_vacancy_state(profile_id,vacancy_id,feedback) "
        ...
        assert (
            execute("SELECT draft_content,status,version FROM school_applications")
            == before
        )
        assert execute(
            "SELECT feedback,feedback_recorded_at,feedback_implicit FROM school_applications"
        ) == [(None, None, [])]
        assert execute(
            "SELECT feedback,feedback_recorded_at FROM profile_vacancy_state"
        ) == [("thumbs_up", None)]
        ...
        for assignment in (
            ...
            assert (
                failure.returncode != 0 and b"school feedback remains" in failure.stderr
            )
        ...
        execute(
            "UPDATE school_applications SET feedback=NULL,feedback_recorded_at=NULL,feedback_implicit='[]'"
        )
        execute(
            "UPDATE profile_vacancy_state SET feedback_recorded_at=clock_timestamp()"
        )
        failure = run_alembic(url, "downgrade", "core0047", check=False)
        assert (
            failure.returncode != 0
            and b"vacancy feedback authority remains" in failure.stderr
        )
