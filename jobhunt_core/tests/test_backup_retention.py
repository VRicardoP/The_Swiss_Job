...
spec = importlib.util.spec_from_file_location(
    "backup_retention", Path(__file__).resolve().parents[2] / "scripts/backup_retention.py"
)
...
def registered(tmp_path, name, *, created=0, verified=True):
    ...
    return {"path": str(path), "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
            "size": path.stat().st_size, "created_at": created, "expires_at": created + 86400,
            "kind": "backup", "database": "synthetic", "verified": verified}
...
def test_registration_cannot_extend_retention_or_replace_an_artifact(tmp_path):
    ...
    value = retention.register(registry, old["path"], kind="temporary",
                               database="synthetic", created_at=0)
    assert registry["entries"] == []
    assert value["entries"][0]["expires_at"] == 48 * 3600
    assert retention.register(value, old["path"], kind="temporary",
                              database="synthetic", created_at=0) == value
    with pytest.raises(ValueError, match="cannot change"):
        retention.register(value, old["path"], kind="temporary",
                           database="synthetic", created_at=86400)


def test_expired_last_backup_is_kept_and_reported_overdue(tmp_path):
    old = registered(tmp_path, "only.dump")
    result = retention.expire({"version": 1, "roots": [str(tmp_path)], "entries": [old]},
                              now=100000, apply=True)
...
def test_corrupt_replacement_cannot_justify_deletion(tmp_path):
    ...
    with pytest.raises(ValueError, match="replacement"):
        retention.expire({"version": 1, "roots": [str(tmp_path)], "entries": [old, new]},
                         now=100000, apply=True)
    assert Path(old["path"]).exists()
