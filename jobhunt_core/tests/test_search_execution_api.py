...
def test_configured_search_invalid_filter_is_400_without_partial_update(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def configure():
        async with factory() as s:
            await search_execution.configure_execution(s, sid, contract=search_execution.CONTRACT,
                notify_since=datetime.now(timezone.utc)-timedelta(days=1))
            await s.commit()
    asyncio.run(configure())
    before = _rows(factory, "SELECT to_jsonb(s) FROM saved_searches s WHERE id=:id", id=sid)
    response = tia._api(factory, f"/v1/saved-searches/{sid}", token=token,
        method="PUT", headers={"If-Match": created.headers["etag"]},
        json_body={"name": "must not persist", "filters": {"unknown_filter": "private value"}})
...
def test_create_execution_is_atomic_and_idempotent_with_detail_etag(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    assert _rows(factory, "SELECT contract,enabled,run_number FROM saved_search_execution WHERE saved_search_id=:id", id=sid) == [("swissjob-v1", True, 0)]
    ...
    assert tia._api(factory, f"/v1/saved-searches/{sid}", token=second_token).status_code == 404
    body["filters"] = {"unexpected": "bad"}
    bad = _post(factory, token, body)
    assert bad.status_code == 400, bad.text
    assert _rows(factory, "SELECT count(*) FROM saved_searches WHERE profile_id=:id", id=pid) == [(1,)]
...
def test_manual_run_requires_authority_owner_delivery_and_queue(db, monkeypatch):  # noqa: F811  (la fixture, no una redefinición)
    ...
    created = _post(factory, token, {"profile_id": str(pid), "name": "run", "execution_contract": "swissjob-v1"})
    ...
    assert tia._api(factory, endpoint, token=token, method="POST").status_code == 503
    monkeypatch.setattr(settings, "CORE_DELIVERY_HTTP_DESTINATIONS", {tenant: "http://never-called.invalid"})
    ...
    send.assert_called_once_with("jobhunt.searches.run_one", kwargs={"search_id": sid}, retry=False)
    ...
    assert _rows(factory, "SELECT last_run_at,total_matches FROM saved_searches WHERE id=:id", id=uuid.UUID(sid)) == [(None, 0)]
