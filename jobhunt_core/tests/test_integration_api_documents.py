...
def _post(factory, token, pid, body=None, key="auto"):
    headers = {} if key is None else {"Idempotency-Key": str(uuid.uuid4()) if key == "auto" else key}
    return tia._api(factory, _path(pid), token=token, method="POST", headers=headers,
                    json_body=body if body is not None else {
                        "doc_type": "cv", "content": '{"name": "Résumé"}',
                        "language": "fr", "source_ref": "local-application",
...
def test_documents_roundtrip_replay_and_content_free_receipt(db):
    ...
    receipt = _rows(f, "SELECT response FROM idempotency_records WHERE key='document-1'")[0][0]
    assert receipt["body"] == {"id": doc["id"]}  # no second copy of the CV
    conflict = _post(f, token, pid, {"doc_type": "cv", "content": "different"}, "document-1")
    assert conflict.status_code == 409
    get = tia._api(f, _path(pid, doc["id"]), token=token)
    assert get.json() == doc
    cached = tia._api(f, _path(pid, doc["id"]), token=token,
                      headers={"If-None-Match": get.headers["etag"]})
    assert cached.status_code == 304
    events = _rows(f, "SELECT o.payload,d.destination FROM integration_outbox o "
                     "JOIN integration_outbox_deliveries d USING(event_id) "
                     "WHERE o.subject_profile_id=:p AND o.type='document.changed'", p=pid)
...
def test_documents_delete_never_replays_erased_content(db):
    ...
    stale = tia._api(f, url, token=token, method="DELETE", headers={"If-Match": '"wrong"'})
    assert stale.status_code == 412
    delete = tia._api(f, url, token=token, method="DELETE",
    ...
    assert tia._api(f, url, token=token, method="DELETE",
                    headers={"Idempotency-Key": "delete-1"}).status_code == 204
    assert _rows(f, "SELECT version FROM integration_outbox WHERE subject_profile_id=:p "
...
def test_documents_ownership_scopes_and_immutable_body(db):
    ...
    assert tia._api(f, _path(pid, did), token=token, method="PUT", json_body={}).status_code == 405


@pytest.mark.parametrize("bad", [
    {"content": "bad\x00content"}, {"context": {"nested": "bad\x00value"}},
...
def test_documents_require_key_and_pagination_has_no_ghost_page(db):
    ...
    page = tia._api(f, _path(pid) + "?limit=2&source_ref=local-application", token=token).json()
    assert len(page["items"]) == 2 and page["next_cursor"]
    following = tia._api(f, _path(pid) + "?limit=2&cursor=" + page["next_cursor"], token=token).json()
