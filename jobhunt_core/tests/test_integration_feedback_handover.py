...
def source_row(factory, pid, vid, **overrides):
    url = rows(
        factory,
        "SELECT url FROM source_listing_incarnations WHERE vacancy_id=:v LIMIT 1",
        v=vid,
    )[0]["url"]
    return {
        "id": str(uuid.uuid4()),
...
def test_feedback_plan_import_replay_reverse_preserves_other_state(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def run():
        async with factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE profile_vacancy_state SET notes='keep',saved_at=now() WHERE profile_id=:p AND vacancy_id=:v"
                ),
                {"p": pid, "v": vid},
            )
            await s.commit()
            before = (
                await s.execute(
                    sa.text(
                        "SELECT to_jsonb(p) FROM profile_vacancy_state p WHERE profile_id=:p AND vacancy_id=:v"
                    ),
                    {"p": pid, "v": vid},
                )
            ).scalar_one()
            ...
            await (
                s.commit()
            )  # In the command, the private plan is fsynced HERE, before apply.
            ...
            assert (
                await s.scalar(
                    sa.text(
                        "SELECT count(*) FROM profile_vacancy_events WHERE profile_id=:p AND kind='implicit'"
                    ),
                    {"p": pid},
                )
                == 2
            ...
            assert (
                await s.scalar(
                    sa.text(
                        "SELECT count(*) FROM profile_vacancy_events WHERE profile_id=:p"
                    ),
                    {"p": pid},
                )
                == 0
...
def test_feedback_clear_is_migrated_and_new_edit_prevents_old_rollback(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def run():
        async with factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE profile_vacancy_state SET feedback='thumbs_down',dismissed_at=now() WHERE profile_id=:p AND vacancy_id=:v"
                ),
                {"p": pid, "v": vid},
            )
            await s.commit()
            ...
            assert await s.scalar(
                sa.text(
                    "SELECT feedback IS NULL AND dismissed_at IS NULL FROM profile_vacancy_state WHERE profile_id=:p AND vacancy_id=:v"
                ),
                {"p": pid, "v": vid},
            )
            await s.execute(
                sa.text(
                    "UPDATE profile_vacancy_state SET feedback='applied' WHERE profile_id=:p AND vacancy_id=:v"
                ),
                {"p": pid, "v": vid},
            )
            await s.commit()
    ...
    async def run():
        async with factory() as s:
            ...
            with pytest.raises(FeedbackMigrationError, match="no core identity"):
                await plan_for(
                    s, pid, [{**first, "url": "https://missing.example/no-listing"}]
                )
