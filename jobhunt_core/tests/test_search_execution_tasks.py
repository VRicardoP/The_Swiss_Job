...
def test_failed_search_does_not_starve_healthy_search_or_consume_observations(
    ...
    async def go():
        async with factory() as s:
            ...
            monkeypatch.setattr(
                ...
                {
                    bad_tenant: "http://never-called.invalid",
                    good_tenant: "http://never-called.invalid",
                },
            )
            ...
            assert (
                await s.scalar(
                    sa.text(
                        "SELECT last_attempt_at FROM saved_search_execution WHERE saved_search_id=:id"
                    ),
                    {"id": bad},
                )
                is not None
