    ...
    try:
        ...
        response = _post(
            ...
            pid,
            {
                ...
                "content": "Permanent original",
                "offer_revision_id": str(oid),
                "context": {"job_title": "Original title"},
            },
        )
        ...
        with pytest.raises(sa.exc.DBAPIError, match="immutable"):
            asyncio.run(
                execute(
                    "UPDATE generated_documents SET content='changed' WHERE id=:d",
                    d=did,
                )
            )
        ...
        row = _rows(
            f,
            "SELECT content,output_hash,offer_revision_id,context "
            "FROM generated_documents WHERE id=:d",
            d=did,
        )[0]
        assert row.content == response.json()["content"]
        assert row.output_hash == response.json()["output_hash"]
        assert row.offer_revision_id is None and row.context == {
...
def test_documents_missing_offer_does_not_write(db):  # noqa: F811  (la fixture, no una redefinición)
    ...
    response = _post(
        ...
        pid,
        {
            "doc_type": "cv",
            "content": "cv",
            "offer_revision_id": str(uuid.uuid4()),
        },
    )
