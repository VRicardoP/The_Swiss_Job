...
def test_native_scopes_admit_refresh_and_preserve_raw(db, source):  # noqa: F811  (la fixture, no una redefinición)
    ...
    async def run():
        ...
        params = (
            {"remote": "Kun hjemmekontor"} if source == "nav_arbeidsplassen" else {}
        )
        async with factory() as session:
            await session.execute(
                sa.text("UPDATE sources SET name=:n WHERE id=:s"),
        ...
        async with httpx.AsyncClient(
            ...
            result = await run_scope(
                sid, get_provider(source), RawListingSink(), client, factory
            )
        ...
        async with factory() as session:
            ...
            if source == "jobgether":
                assert (
                    rows[0].content["description"] is None
                )  # sink canonizes absent text
