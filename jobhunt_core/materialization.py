...
async def materialize_misses(session_factory, profile_id, model_id, policy_id,
                             budget_seconds=3600.0, batch_pairs=8):
    ...
    def backlog():
        return {"scored": scored, "remaining": pending, "agotado": True, "status": "backlog"}
            ...
            while True:
                ...
                async with session_factory() as session:
                    computed = await matching.compute_policy_feed(
                        ...
                        exclude_dismissed=False, ce_inference=False,
                    )
                    if computed["status"] == "ok_prep":
                        consumer = (await session.execute(sa.text(
                            "SELECT c.name FROM profiles p JOIN consumers c "
                            "ON c.id = p.consumer_id WHERE p.id = :pid"
                        ), {"pid": profile_id})).scalar_one()
                cost_prep = max(cost_prep, time.monotonic() - prep_started)
                if computed["status"] != "ok_prep":
                    return {"scored": scored, "remaining": 0, "agotado": False,
                            "status": computed["status"]}
                prep = computed["prep"]
                misses = prep["misses"]
                pending = len(misses)
                if not misses:
                    return {"scored": scored, "remaining": 0, "agotado": False, "status": "ok"}
                offset = 0
                while offset < len(misses):
                    n = min(1 if not cost_doc else min(batch_pairs, 8), len(misses) - offset)
                    if remaining() <= 0 or (cost_doc and n * cost_doc * 1.25 > remaining()):
                        return backlog()
                    batch = misses[offset:offset + n]
                    batch_prep = dict(prep, misses=batch, candidates=batch, cache={},
                    ...
                    async with session_factory() as session:
                        await matching._persist_eval_rows(
                            session, profile_id, rows, computed["profile_revision_id"],
                            model_id, policy_id, consumer,
                        )
