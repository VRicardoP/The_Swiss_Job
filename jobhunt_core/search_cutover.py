...
def read_snapshot(path):
    ...
    if snapshot.get("version") != 1 or digest(
        {k: v for k, v in snapshot.items() if k != "seal"}
    ) != snapshot.get("seal"):
...
async def verify_source(source, markers, snapshot):
    ...
    if (
        await source.scalar(sa.text("SELECT current_database()"))
    ...
    rows = [
        ...
        for row in (
            await source.execute(
                sa.text("SELECT * FROM public.saved_searches ORDER BY id")
            )
        ).mappings()
    ]
    ...
    links = dict(
        (
            await source.execute(
                sa.text(
                    "SELECT user_id::text,core_profile_id::text "
                    "FROM public.jobhunt_profile_map WHERE user_id=ANY(:ids)"
                ),
                {"ids": [uuid.UUID(uid) for uid in snapshot["bindings"]]},
            )
        ).all()
    )
    ...
    fingerprint = dict(
        (await source.execute(sa.text(JOBS_FINGERPRINT_SQL))).mappings().one()
    )
    ...
    for sid, values in snapshot["sent"].items():
        ...
        if len(actual) != len(keys) or any(
            value not in (None, b"1") for value in actual
        ):
...
async def run(args):
    ...
    markers = redis.from_url(
        os.environ["SOURCE_REDIS_URL"], socket_connect_timeout=2, socket_timeout=2
    )
    try:
                ...
                for sid, candidates in snapshot["candidates"].items():
                    for row in candidates:
                        if not snapshot["sent"][sid][row["hash"]]:
                            previous = grouped[row["source"]].setdefault(
                                row["hash"], row
                            )
                            if previous != row:
                                raise SearchMigrationError(
                                    "inconsistent captured offer"
                                )
                results = []
                for index, (name, rows) in enumerate(sorted(grouped.items())):
                    async with factory() as core, core.begin():
                        result = await seed_missing_offers(
                            core, name, list(rows.values())
                        )
                        receipt = {
                            "source": name,
                ...
                async with factory() as core, core.begin():
                    ...
                    if args.command == "plan":
                        plan = await prepare_plan(
                            ...
                            rows=snapshot["rows"],
                            targets=private_read(args.targets),
                            pending=await resolve_pending(core, snapshot),
                        ...
                        result = {
                            "operation": "plan",
                            "seal": plan["seal"],
                            "searches": len(plan["after"]),
                        }
                    else:
                        plan = private_read(args.plan)
                        if plan.get("snapshot_seal") != snapshot["seal"]:
                            raise SearchMigrationError(
                                "plan belongs to another snapshot"
                            )
                        result = await apply_plan(
                            core, plan, reverse=args.command == "revert"
                        )
        ...
        return {
            ...
            "verified": True,
            "inserted_listings": sum(
                r["inserted_listings"] for r in result.get("sources", [])
            ),
        }
    finally:
