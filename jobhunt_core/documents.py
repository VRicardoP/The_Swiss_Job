...
_FROM = ("FROM generated_documents d JOIN profiles p ON p.id=d.profile_id "
...
async def owner(session, profile_id, consumer_id, *, write=False):
    ...
    ), {"pid": profile_id, "cid": consumer_id})).scalar_one_or_none()


async def fetch(session, profile_id, document_id, consumer_id):
    return (await session.execute(sa.text(
        "SELECT d.* " + _FROM + " AND d.id=:did"
    ), {"pid": profile_id, "did": document_id, "cid": consumer_id})).mappings().one_or_none()
...
async def page(session, profile_id, consumer_id, limit, cursor, doc_type, source_ref):
    ...
    rows = (await session.execute(sa.text(
        sql + " ORDER BY d.created_at DESC,d.id DESC LIMIT :lim"
    ), params)).mappings().all()
    items = rows[:limit]
    following = (items[-1]["created_at"], items[-1]["id"]) if len(rows) > limit else None
...
async def emit_changed(session, profile_id, document_id, destination, *, deleted=False):
    ...
    await outbox.emit(
        session, event_type="document.changed", natural_key=f"{document_id}:{version}",
        aggregate="document", aggregate_id=str(document_id), subject_profile_id=profile_id,
        version=version, destination=destination,
        payload={"document_id": str(document_id), "profile_id": str(profile_id),
...
async def create(session, profile_id, values, destination):
    ...
    if values["offer_revision_id"] is not None:
        offer = (await session.execute(sa.text(
            "SELECT id FROM offer_revisions WHERE id=:id FOR KEY SHARE"
        ), {"id": values["offer_revision_id"]})).scalar_one_or_none()
        if offer is None:
            return None
    ...
    ), {**values, "id": document_id, "pid": profile_id,
...
async def delete(session, profile_id, document_id, destination):
    ...
    await session.execute(sa.text(
        "DELETE FROM generated_documents WHERE id=:did AND profile_id=:pid"
